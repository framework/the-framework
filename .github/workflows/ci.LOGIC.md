...
2. `pnpm build`: every package the daemon runs, in dependency order, the same list `clean` and `test` name: `agent-data`, `skill-branches`, `skill-github`, `skill-tickets`, `skill-queue`, `skill-logs`, `skill-browser`, `skill-discord`, `agent-driver/core`, `agent-driver/claude`, `agent-driver/codex`, `agent-driver/github`, `agent-runner`, `agent-scheduler`, then the framework, whose build compiles it, copies the browser extension into it (`packages/framework/scripts/copy-extension.mjs`) and builds the dashboard. Packages the framework does not import, `agent-runner` and `agent-scheduler` among them, are built all the same: the project's hooks the daemon runs start them.
3. `pnpm typecheck`: the same twelve packages (each skill package's dashboard included), then the website package.
4. `pnpm test`: the tests of every package, in order: `agent-data`, `skill-branches`, `skill-github`, `skill-tickets`, `skill-queue`, `skill-logs`, `skill-browser`, `skill-discord`, `agent-driver/core`, `agent-driver/claude`, `agent-driver/codex`, `agent-driver/github`, `agent-runner`, `agent-scheduler`, then `framework`.
