...
- The branch-management skill: every agent's system prompt carries the `@better-skills/branch-management` package's `SKILL.md` — start inside your own checkout and never edit the repository around it, name the session, commit as you go, leave a clean tree, never push or open the PR yourself
...
- The session name is the branch: an agent is labelled by its `tf-<name>` branch, read from git after every settled turn — nothing to signal, nothing to record twice
