...
- **How the packages fit together** - one product package on top of six libraries it is built from, plus the runner and the scheduler, the browser and Discord skills, the command [9] packages and two companions; the product is a dashboard over what the others do.
...
- **While nobody is at the keyboard** - the scheduler works the queue and keeps it fed, standing down at the quota boundary [14]; the daemon itself only keeps every project's `agent-data` branch [8] in step with the remote, serves the feeds an open dashboard alerts from, and tidies what cloud sessions left behind.
...
- `packages/framework` — the product: the `the-framework` CLI (four options, no verbs), the daemon it runs in the foreground (Ctrl-C closes the dashboard; an agent [1] is not the daemon's process and goes on to its end), and the dashboard the daemon serves — the product's only user interface. It runs no coding agent [2] and names no tool: it starts an agent through the project's start hook [4] and reads the agent's card and diary [12] for everything it shows. It ships no prompt text. Depends on all six libraries below (of the driver family, `agent-driver` and `@agent-driver/claude`).
- `packages/agent-driver` — the driver family: `core/` is the driver seam: one contract for driving a coding agent [2] as a black box, with a fake implementation; the real ones are adapters in their own packages beside it under `packages/agent-driver/`: `@agent-driver/claude` (Claude Code on this machine, and the account's quota), `@agent-driver/codex` (Codex on this machine), each turning off the person's own setup with its coding agent's switches, and `@agent-driver/github-actions` (a coding agent on a GitHub Actions runner, Claude Code today); it keeps the agent's card and diary [12], reads the inbox [13] when a turn ends, and reports the question [5] a turn ended on. The Framework prompts one turn at a time, lets the coding agent's own loop run to completion, and learns everything from the turn's final message: it never gates on the agent's individual tool calls, holds no model key, and runs on the user's own subscription.
...
- `packages/skill-browser` — the `browser` skill: a headless Chrome of the agent's own, on an empty profile, driven from the shell (`browser open`, `read`, `click`, `type`, `press`, `screenshot`, `eval`, `close`) through one background process per project that closes it when the agent [1] ends, when it is closed, or after 30 minutes unused. When the agent's environment names its diary [12], opening a page appends a `screen` line to that diary, and the dashboard shows the browser live in the agent's transcript, where the agent opened it, for the person watching to click and type in too. Unlike the four skills above it keeps nothing on the `agent-data` branch [8] and the product does not depend on it; it depends on nothing.
- `packages/skill-discord` — the `discord` skill: one message posted to a Discord channel from the shell (`discord send`) through the channel's webhook, which a person gives each machine once (`discord setup`) and which is saved for that user outside every project; `DISCORD_WEBHOOK` in the environment wins over it. A message goes out formatted by Discord's markdown, cut to 2,000 characters, and pings nobody. It is how a person hears on Discord that a run needs them: the runner's `ended:` line, which the person writes, runs `npx discord send "$MESSAGE"`. Like the browser skill it keeps nothing on the `agent-data` branch [8], the product does not depend on it, and it depends on nothing.
- `packages/skill-question` — the `question` skill: one skill file and no code that teaches an agent how to ask the person to decide: when a choice is theirs, end the reply with one `await-choices` block, the question [5] with its options, and stop; an agent with its own tool for asking uses that instead, as Claude Code does in a terminal, so the block is written by agents a runner started. The agent picks it by itself; the launcher does not list it, and nothing depends on it.
...
- `packages/skill-update-tickets`, `packages/skill-plan-tickets`, `packages/skill-triage` — the routine commands, one package each of the same shape: bring the tickets up to date with the issue tracker, queue a plan for the tickets with none or an outdated one, ten a run at most, queue the tickets whose plan says they are ready (the quick wins and the consensual work, two modes of one command, each on its own schedule line). The schedule says how often at most each runs, and only while a check finds work.
...
- `packages/agent-runner` — the runner: a tool like `agent-driver`, one command line, that runs one agent from start to end in its own checkout [3] — one prompt to the coding agent [2], the agent's own loop to its end, the run [11] recorded on the `agent-data` branch [8], the checkout reclaimed — continues one that ended on a question, and sweeps up the runs of this machine whose process died. When a run ends waiting on a question, or ends done with a pull request it did not have, it runs the person's `ended:` line from the project's `.agent-runner/config.yml` (on this machine only), a shell line naming no service of the tool's. Its command line is what a project's start hook and resume hook [4], and its `check` line, name in this monorepo. Reads no schedule and keeps no state. Depends on `agent-driver`, `@agent-driver/claude` and `@agent-driver/codex`, `agent-data`, the branches and the logs packages; never on The Framework, which never depends on it.
- `packages/agent-scheduler` — the scheduler: a tool like `agent-driver`, one command line and one small process per project, that reads the project's tracked `agent-schedule.md` (per command [9] a check that says there is work, an interval that says how often at most, or both, and `off` for a command that runs only on a machine where a person switched it on), ticks every minute, and has the runner start one agent per due command, counting the runs [11] in flight on the `agent-data` branch against each command's cap. Its command line is what a project's `open`, `close`, `offset` and `switch` lines in `.the-framework/hooks.yml` name in this monorepo. Depends on the runner, `agent-driver` and its Claude Code adapter, `agent-data`, the branches and the logs packages; never on The Framework, which never depends on it.
...
The dashboard is a projection of those files throughout. It tails the agent's diary to the browser as the agent writes it and reads the recorded diary long afterwards, so watching now and reading later show the same record. A command the agent runs can append lines of its own to the diary, whose path the tool running the agent puts in the agent's environment: a `screen` line puts a live page from this machine, such as the browser skill's browser, into the agent's transcript where the agent used it, until the command says it has gone or the agent ends.
...
The daemon's own background work starts no agent. On one shared clock it keeps every project's `agent-data` branch [8] in step with the remote and records what a project cannot converge, and tidies what cloud sessions left behind: it adopts the branch a session pushed by its ancestry from the cloud anchor [18], and expires dead cloud refs. It serves two feeds — what needs a human (an open question, a pull request to review, unpushed commits) and plain activity — which an open dashboard page polls, alerting in the browser on what is new; with no dashboard open, nothing alerts. The daemon posts nowhere: a person hears on Discord that a run needs them through the runner's `ended:` line and the `discord` skill.
