...
- **The product** (`framework/`) - the `framework` npm package: the `the-framework` CLI, the daemon, the agent [1] lifecycle, the dashboard it serves, and every prompt an agent is sent. Depends on the driver, the branch store and the four skills below; not on the command skill, not on the scheduler.
