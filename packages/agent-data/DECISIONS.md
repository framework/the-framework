...
- Read from anywhere in the repository, no checkout needed: a file comes from the
  ...
  skip the checkout, prefer origin's, for a reader about to act on the files when its
  checkout may trail what others pushed. A one-shot command opens the branch once: one
  fetch, then every read off that one ref (origin's copy, or the local branch when origin
  has none); its own writes go straight to the remote, so the local branch trails what it
  just wrote.
...
- Two writers. A long-running process (a daemon, the one that starts agents) writes in its
  ...
  same queue: it is that same cycle with an empty change. A command an agent runs writes
...
- A write is handed over as a small function ("add this line"), not as a finished
  commit, so starting over is just running it again on the new files. The commit message
  is the caller's too: fixed, or a function run after the change, since a write that
  batches several edits only knows what it did once it is done. The lost attempt's
...
- The remote is always `origin`. No remote: the process's write commits locally and
  reports no error; a command's write refuses; the pull reports an error: it has no remote
  to converge with.
