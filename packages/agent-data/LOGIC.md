...
- **Who provides a kind of data** (`src/provided-command.ts`) - which of a project's installed packages declares that its command answers a kind of The Framework's data, the project's own line naming one when two declare the same kind, and how such a command is run; shared so the product and the scheduler resolve it the same way.
