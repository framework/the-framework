...
- **A branch used as a file store** (`src/`, `src/file-branch.ts`, `src/checkout-lock.ts`) - checked out at `.branches/<branch>` and hidden from git, born an orphan or adopted from origin, written one cycle at a time across every process on the clone (sync, apply, commit, push) with the change re-applied when the push loses a race and the remote winning a conflict, never force-pushed, pulled on the daemon's clock, read from anywhere without ever failing, and written one-shot from any clone through a throwaway checkout.
...
- **Who provides a kind of data** (`src/provided-command.ts`) - which of a project's installed packages declares that its command answers a kind of The Framework's data, the project's own line naming one when two declare the same kind, and how such a command is run; shared so the product and the runner (`agent-runner`) resolve it the same way.
