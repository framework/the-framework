...
- **Taking turns in the checkout** (`checkout-lock.ts`) - one lock file per checkout, `.branches/<branch>.lock`, that every process on the clone takes before a write cycle, a pull or a setup: a live holder is waited for, five minutes at most; a gone holder's lock is taken over.
- **Who provides a kind of data** (`provided-command.ts`) - the project's installed packages, which of them declares that its command answers a kind of The Framework's data, the project's own line naming one when two declare the same kind (else none, with the reason), and how such a command is run in the project and its JSON read.
- **The entry point** (`index.ts`) - what every skill and the product import.
- **The tests** (`git.test.ts`, `file-branch.test.ts`, `provided-command.test.ts`) - the budgets and failure readings of the git runner; the whole life of the branch against real repositories: birth, adoption, writes, stranded commits, conflicts, lost races, the pull, serialization within a process and across two, the checkout's lock, reads, and one-shot writes; who provides a kind against real package files.
