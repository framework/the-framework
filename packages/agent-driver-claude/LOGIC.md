...
- **Claude Code, on this machine and on a runner** (`src/`) - one `claude` process per turn, its streamed output read, its conversation resumed; the person's own setup turned off part by part; whether `claude` is installed and logged in; the same Claude Code on a GitHub Actions runner through a workflow; and the account's quota [1]; told in `src/LOGIC.md`.
