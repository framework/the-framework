...
- **Claude Code on a GitHub Actions runner** (`actions.ts`, `actions.test.ts`) - each turn dispatches the agent workflow with the prompt as an input, finds its run by a correlation id [3], waits up to 1 hour, reads the transcript back from the run's artifact and replays it in a burst; continuity across turns is one branch the driver names and every run pushes to. The GitHub Actions runner holds no personal setup, so there is nothing to turn off.
...
- **The entry point** (`index.ts`) - both drivers with their sessions and option types, Claude Code's output parser and the Actions transcript replay, the readiness check and the quota reader; the zip reader stays internal.
