The driver family: one contract for driving a coding agent [1] as a black box, and one package per driver [2] on it, all under this folder. A driver is either a coding agent on this machine or a place a coding agent runs elsewhere.

## Context

**Business logic story**: two things vary independently: which coding agent [1] works (Claude Code, Codex) and where it runs (this machine, a GitHub Actions runner, the vendor's cloud). What depends on the coding agent is its command line, the reader of its output, its switches for the person's own setup and its login question; what depends on the place is how a turn reaches it and comes back. So a package is one of the two, and a place package takes the coding agent's output reader from that coding agent's package.

| | this machine | a GitHub Actions runner | the vendor's cloud |
|---|---|---|---|
| Claude Code | `claude/` | `github/` | Claude Code in a cloud session, still in the product (`packages/framework`) |
...
- **A coding agent on a GitHub Actions runner** (`github/`) - the `@agent-driver/github` npm package: each turn one run of the project's agent workflow, its transcript read back and replayed; Claude Code today, read with `@agent-driver/claude`'s output reader; told in `github/LOGIC.md`.
