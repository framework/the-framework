The `agent-driver` package is how The Framework works a repository without ever calling a model itself: it wraps a coding agent [1] the user already pays for as a driver [2], a black box started in a directory, prompted for one turn [3] at a time, streamed as progress events [4] and resumed later, and ships the pieces every implementation shares and a scripted fake for tests and offline demos. The real implementations are adapters, each its own package on this contract beside this one: `@agent-driver/claude` (`../claude`) for Claude Code on this machine, `@agent-driver/codex` (`../codex`) for Codex on this machine, `@agent-driver/github-actions` (`../github-actions`) for a coding agent on a GitHub Actions runner; the product's own implementation, Claude Code in a cloud session [6] (`claude-web`), stays in the product for now, since it runs through the product's browser extension. The product's agent [5] lifecycle in `packages/framework` speaks only this contract, which is what lets it implement Claude Code in a cloud session [6] itself and swap one coding agent for another without changing anything above. `package.json`, `tsconfig.json`, `tsconfig.build.json` and `tsconfig.test.json` configure the build and the test runner and carry no business logic; `dist/` and `dist-test/` are build output; each source file's `*.BUG-ANALYSIS.md` records when it was last reviewed for bugs.
...
- **The driver seam and what adapters share** (`src/`) - the contract every driver [2] honors, including the three parts of the person's own setup every adapter takes, the process core shared by the local implementations with its stop and reaping rules, the fake implementation, the question an agent asks, the inbox that reaches a running agent, the log a session keeps, and the check that a coding agent's CLI is installed and logged in, which each adapter runs with its own CLI's questions; told in `src/LOGIC.md`.
