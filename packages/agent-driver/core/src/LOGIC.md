...
[7] usage: what one turn spent, as the coding agent reports it: token counts, and a notional price in US dollars when the coding agent prices its turns.
[8] rate limit: the coding agent's per-turn reading of whether the account may still spend against one quota window, and when that window resets.
...
[13] live chat: the user's own messages to a running agent, each continuing the same driver session. One of them is a message.
[14] question: the fenced `await-choices` block an agent ends a turn with when it will not decide alone: a title, options, a recommended one.
[15] inbox: a file of lines from outside the agent, messages and answers, sent into the session when a turn ends.
[16] log: the card and the diary of one driver session, two files in a run record's shape at a directory the caller gives.
[17] personal setup: the three parts of the person's own setup a coding agent loads when started by hand, by the names every adapter takes: `memory` (what the coding agent remembers across sessions on its own), `connectors` (the apps and accounts linked to the person's login), `skills` (the person's own instructions, skills and settings files).

## Business logic — TL;DR

- **The contract** (`types.ts`) - what every driver [1] promises, the three parts of the personal setup [17] every adapter takes (each on or off, a part an adapter cannot turn off said in its readiness check, all on when none is given), how a driver session [9] is started (with a log [16] when asked), what one turn [3] returns (draining an inbox [15] first when named), the nine kinds of progress event [4], the three readings of spend (usage [7], rate limit [8], quota [6]), and which reasons for an empty quota reading are transient.
- **The question** (`question.ts`, `question.test.ts`) - the one parser of the block an agent ends a turn with when it asks [14], tolerant, and the continuation prompt an answer resumes the agent with.
...
- **What every driver session shares** (`session-support.ts`) - reporting progress events without letting a listener break the coding agent [2], folding the driver session's stop request [11] and framing [10] with a turn's own, and reading a file out of the directory.
...
- **Can a session start here** (`ready.ts`, `ready.test.ts`) - before a run spends a checkout [12], the coding agent's [2] CLI is asked whether it is installed and logged in, with the questions and answers an adapter hands in: a missing or logged-out CLI is a problem naming its fix, an answer that cannot be read passes, and running as root is a warning; an adapter adds its own warnings, such as a part of the personal setup [17] it cannot turn off.
...
- **Where the implementations differ** - how each implementation starts its coding agent, delivers framing, resumes a conversation, reports spend and quota, reads code back, turns off the personal setup [17], and whose login it spends; the log, the question and the inbox they all share.
...
The Claude Code and Codex adapters each supply only two things for a turn [3]: the command line to run and the parser that understands their coding agent's [2] output. Everything about the process is shared (`cli-session.ts`, here): the coding agent is spawned in the driver session's [9] directory, the agent's [5] checkout [12], as the leader of its own process group; the prompt is fed over standard input; a `start` progress event [4] announces the turn; every output line goes through the driver's parser and its progress events are forwarded through the isolated reporter (`session-support.ts`) as they come; and the exit code decides the turn, so a crash after streamed text is a failed turn. While the process lives its group is registered (`child-registry.ts`), so a stop request [11] terminates the whole tree at once, with a forced kill 5 seconds later, and a hard exit of The Framework's process kills every registered tree on the way out. A turn has no time limit of its own on this machine; only the GitHub Actions driver gives up waiting, after 1 hour.

### Where the implementations differ

#### Context

**Business logic story**: the contract is one, the coding agents [2] are not. The differences below are each coding agent's own business; the caller sees the same driver session [9], turns [3] and progress events [4] whichever implementation carries the agent [5].
...
- **How framing reaches the coding agent**: Claude Code takes the framing [10] as an addition to its system prompt; Codex and the GitHub Actions workflow take it ahead of the prompt as its own block; the fake ignores it.
...
- **What is spent**: Claude Code reports tokens and a price; Codex reports tokens and no price, never zero; only Claude Code reports rate limit [8] readings, and only the `claude-code` implementation reads the account's quota [6]; Codex, the GitHub Actions implementation and the fake offer no quota reading rather than a made-up number.
