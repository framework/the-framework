...
- **The shared turn end** - like every driver, a fake turn ends with the question reported and the inbox drained, and a fake session keeps the log when asked, so the transport and the log are tested with no process.
