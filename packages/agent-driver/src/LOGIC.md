...
- **The log** (`session-log.ts`) - the card and the diary in a run record's shape, written as events arrive, patched, ended and reopened by the caller; the Claude Code and Codex implementations hand the diary's path to the coding agent as `AGENT_DIARY`, so a command it runs can append its own lines.
