...
- The caller gets the same event stream as a local agent — the agent's text, its tool calls, and a link to the run — all arriving at once when the run completes.
