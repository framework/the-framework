The fake driver: a driver that answers from a script instead of from a coding-agent CLI, so a whole product can run offline — no CLI installed, no model called, no cost, and the same outcome every time.

## User story

Someone wants to try the caller's product, or to test it, without a coding-agent subscription. Its UI, its agents, its gates, and its event streams all behave as they normally do; only the agent's answers are canned.
...
- **The event stream is indistinguishable from a real one** - the same start, tool-call, text, and result events reach the caller, including any usage figures the script names, so every surface can be exercised end to end.
