...
The one rule worth stating: a surface that fails while handling an agent's event — the caller's UI throwing, say — never takes the agent down with it. The failure is logged, naming which driver's event it happened on, and the agent carries on.
