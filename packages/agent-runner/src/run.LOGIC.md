...
[1] run: one agent this tool starts: a process of the tool's own (`agent-runner run`), a checkout, one prompt to the coding agent, and a run record when it ends. Its id is its start time with `:` and `.` replaced by `-`, `2026-09-16T14-01-00-000Z`, the shape the dashboard sorts runs by.
...
[6] the tool's mark: `caller.runner` on a card: the machine that started the run, the run's process on that machine while it runs, and the follow-up's [11] prompt when the run names one.
...
[13] the `ended:` line: the value of the `ended` key in the project's `.agent-runner/config.yml`, on this machine only, a shell command of the person's that this tool runs when a run ends waiting, or ends done with a new pull request (`ended.ts`).

## Business logic — TL;DR

- **The id and the mark** - the id is given by the process that spawned the run (a detached start, a scheduler's tick) or minted from the start time; the mark names this host and this process, and the follow-up [11] when the run names one; it names no command: the card's intent is the prompt, and a scheduler reads off it which of its commands the run counts for.
...
- **The `ended:` line** - once the record is written, before the reclaim, the person's `ended:` line [13] runs (`ended.ts`) when the run ended `waiting`, or ended `done` with a pull request other than the one its branch already had when this process took it up (the first run's, for a follow-up's run); waited for while the run's lock is held, so a resume started meanwhile waits too, and never changing how the run ended.
- **The reclaim** - the checkout reclaimed under the `branches` rule, pushing on the way; a dirty tree or a branch that could not be pushed keeps it, with the reason in the outcome, and the sweep tries again later.
...
Before anything else, the run's lock [10] is taken for the run's id with this process's pid (`run-lock.ts`): while another live process holds it, this one waits, looking again every 500 ms. A run spawned by `run --detach` or by a scheduler's tick finds the lock already handed to its own pid by the process that spawned it, and goes on at once. The lock is let go once the run has answered, whether the run ended, failed to get a checkout, or threw; a lock that could not be let go is ignored, since a dead pid holds nothing.
...
The checkout [2] is made by the `branches` package for the run's id: a worktree under `.branches/` on the birth branch `agent-<id>`, with everything an agent needs; or, when the run is given an existing branch (a follow-up's [11] run is given the first run's), a worktree named after the run's id and attached to that branch. When the checkout cannot be made, there is no run: the card is written `failed` with the end time, the diary one `ended` line whose detail is `could not create a checkout: <the error>` and whose time is the end time, and the outcome is `failed` with the same detail and a checkout that was not reclaimed for the reason `no checkout`. The live record's directory is hidden from git in the checkout alone, by a `.gitignore` of `*` inside it, since an untracked directory would keep the checkout dirty and a dirty checkout is never reclaimed; a `.gitignore` already there is kept as it is, so the project's own must ignore the live files, as the dashboard's does. It is never hidden through the repository's exclude file: every checkout reads that file, the project's own included, and the dashboard could no longer track its directory there when it adds the project.
...
The branch is read back from the checkout, the birth branch when it cannot be read. The pull request is read back off that branch through the project's git host [12] (`git-host.ts`), or none: no git host package, or a git host that cannot tell, records none. Both are patched onto the live card; the session's log is ended with the status and, when there is one, the detail, which appends the `ended` line and sets the card's status and end time. Once every write has landed, the lines that came as the run ended are sent (above), which ends the log again each time; the status the record and the reclaim go by is the last end's. Once the inbox is empty, the driver session is disposed, and the card and the diary are read back from the checkout and written onto the branch unchanged, over the marker, same id; a record that could not even be committed is logged as `the run's record could not be written: …`. When the session never opened (the driver threw on start), the card is the starting one with the status and the end time, and the diary one `ended` line with the card's end time as its time (the current time when the card has none), after the diary the run was resumed from, if any.

### The `ended:` line

#### Context

**User story**: the user put `ended: npx discord send "$MESSAGE"` in `.agent-runner/config.yml`; a run that stops on a question, or opens a pull request, posts one line in the team's channel.

**Problem**: a resumed run still on the pull request it already opened has nothing new to tell the person, nor has a follow-up's run on the first run's branch; a fresh run has no earlier pull request.

#### Business logic

Once the record is written (whether or not it could be committed), and before the reclaim, the run's end is handed to `ended.ts`: the run's id, its prompt (the record's intent), the status it ended with, the question's title when the last end was `waiting`, the pull request read back at the last end, and the pull request the branch already had when this process took the run up: the first run's for a follow-up's run [11], the record's for a resume, none for any other fresh run. The `ended:` line [13] runs when the run ended `waiting`, or ended `done` with a pull request whose URL differs from that starting one; a `failed` or `stopped` run, or a `done` one with no new pull request, runs nothing. The run's process waits for the line to exit, and when it is still running at 60 seconds, kills it with everything it started; the run's lock is held all that time, so a resume of the same run started meanwhile waits for the line too; whatever it does, the run's status, record and outcome stay as they were, and a failure of the line is one line on the run's log. When the inbox made the run end more than once, only the last end counts. A follow-up's [11] run ending done on the first run's pull request runs nothing, so the pull request is announced once, by the run that opened it.
...
- Otherwise a fresh run starts in the same process: a new id minted from the clock's now, its own marker, its own record and its own lock, a checkout named after its own id and attached to the first run's branch (the branch read back at the first run's end), and the prompt `<follow-up prompt> <first run's id>` (`/post-merge-cleanup 2026-09-16T14-01-00-000Z`). A scheduler counts it, off that prompt, for `post-merge-cleanup`. It runs on the coding agent made for its own id (Claude Code or Codex, the same one as the first run's, `runner.ts`), or on the first run's driver when none is made per id; it gets the first run's model, host, pid and clock. It names no follow-up of its own, so its prompt carries no such line and nothing follows it.
