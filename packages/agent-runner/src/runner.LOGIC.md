...
- **Can a run start here** - the coding agent's readiness from `agent-driver` (its CLI installed and logged in: a problem when not); nothing else is probed; what the `check` command answers, and what a person's run and a scheduler's tick refuse on.
