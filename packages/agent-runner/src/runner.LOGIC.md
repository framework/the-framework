...
- **Can a run start here** - the readiness check of the coding agent's own driver package: problems when its CLI is missing or logged out, warnings for running as root and, on Codex, for skills in `~/.agents/skills` while this machine leaves `skills` out; nothing else is probed; what the `check` command answers, and what a person's run and a scheduler's tick refuse on.
...
### The person's own setup, left out
