Writes this tool's lines into a dashboard's hooks file, `.the-framework/hooks.yml` in the project, so the dashboard's check before a Start, the Start, its answers to a run's question, its opening and closing, its spend-offset slider, and its schedule switches (whether a scheduled command runs on this machine) run through this tool. What `agent-scheduler init` runs.
...
- **The lines** - `open`: `npx agent-scheduler start`; `close`: `npx agent-scheduler stop --unless-keep-alive`; `start`: `npx agent-scheduler run --detach "$PROMPT" ${DRIVER:+--driver "$DRIVER"} ${MODEL:+--model "$MODEL"} ${THEN:+--then "$THEN"}`; `resume`: `npx agent-scheduler run --detach --resume "$RUN_ID" ${TEXT:+"$TEXT"} ${ANSWER:+--answer "$ANSWER"}`; `check`: `npx agent-scheduler check ${DRIVER:+--driver "$DRIVER"}`; `offset`: `npx agent-scheduler offset -- "$POINTS"`; `switch`: `npx agent-scheduler switch "$COMMAND" "$SWITCH"`. `open` and `close` are lists, the others one line each.
