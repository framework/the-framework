...
**Business logic story**: the tick reads the quota through `agent-driver` (which reads Claude Code's own usage readout) only when a command is due, under its cap and about to start, and asks this file once per tick. Copied rather than imported from The Framework because this tool depends on `agent-driver`, which reads the windows, and not on The Framework, which draws the panel.
