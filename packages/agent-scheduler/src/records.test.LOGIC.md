...
- **The last start** - the newest start among a command's cards on any machine, a done and a failed one included, while a running card without the tool's mark and another command's cards do not count; a command never started has none.
