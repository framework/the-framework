...
[12] forge: the service hosting the project's repository and its pull requests, reached through the command the project's forge package declares (`forge.ts`); the tool names no forge.
...
- **The follow-up** - a run naming a follow-up [11] prompts its agent with one more line, "Open the pull request but do not arm its merge: it is merged for you once a follow-up is done." (the record keeps the bare prompt); once it ends done with a pull request, a fresh run with its own id, record and checkout attached to the first run's branch gets the follow-up's prompt and the first run's id, on the coding agent made for its own id; when it ends done, this process merges the first run's pull request through the forge [12]; any other end leaves the request open. A run that ends otherwise, or with no pull request, has no follow-up.
...
**Problem**: the follow-up's work belongs in the same pull request, so the first agent's merge must not land before it; the first agent does not know a follow-up is coming unless told, so its prompt says so, and the merge is this process's, through the forge [12].
...
- When the follow-up's run ends `done`, the first run's pull request is merged through the project's forge (`forge.ts`, `merge <number>`), and the outcome (`auto-armed`, `merged`, `watching`, or `failed` with the reason, "this project has no forge package" among them) is answered with the follow-up's outcome.
