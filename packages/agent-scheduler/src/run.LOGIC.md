...
[12] hold: the `branches` package's mark on a checkout saying more work comes on its branch after the agent: the agent's `publish --merge` from it opens the pull request, arms nothing and records the merge as wanted, and the release arms it later (`merge-hold.ts` there).
...
- **The checkout** - made through the `branches` package for the run's id, on the birth branch `agent-<id>`, or attached to an existing branch when one is given (a follow-up's), and put under the hold [12] when the run names a follow-up [11]; without one there is no run: the record is written `failed` with `could not create a checkout: …` over the marker, and the outcome says `no checkout`; a hold that cannot be put is no run either: `failed` with `could not hold the merge: …`, the checkout reclaimed.
...
The checkout [2] is made by the `branches` package for the run's id: a worktree under `.branches/` on the birth branch `agent-<id>`, with everything an agent needs; or, when the run is given an existing branch (a follow-up's [11] run is given the first run's), a worktree named after the run's id and attached to that branch. When the checkout cannot be made, there is no run: the card is written `failed` with the end time, the diary one `ended` line whose detail is `could not create a checkout: <the error>`, and the outcome is `failed` with the same detail and a checkout that was not reclaimed for the reason `no checkout`. When the run names a follow-up, the checkout is then put under the hold [12]; a hold that cannot be put would let the merge land before the follow-up, so no agent starts: the card is written `failed` with the branch and the end time, the diary one `ended` line whose detail is `could not hold the merge: <the error>`, and the checkout, still empty, is reclaimed as at a run's end. The live record's directory is hidden from git through the checkout's exclude file, since an untracked directory would keep the checkout dirty and a dirty checkout is never reclaimed.
...
**User story**: the user ticks "Post-merge cleanup" in the launcher and starts a run; the agent works and publishes with `--merge`, but its pull request stays open with a line saying the merge is held; a second run appears, on the same branch, running `/post-merge-cleanup` with the first run's id, and adds its knowledge-file changes to the same pull request; once it ends done, the request merges on green.

**Problem**: the follow-up's work belongs in the same pull request, so the first agent's merge must not land before it; the first agent does not know a follow-up is coming, so the merge it asks for is held for it (`branches`' hold [12]), and released by this process.

#### Business logic

A run that names a follow-up [11] runs as any other (above), its checkout under the hold [12]; the follow-up is decided once the run has answered, after its record is written, its checkout reclaimed or kept, and its lock let go:

- The run did not end `done` (failed, stopped, waiting), or no pull request was read back off its branch: nothing follows, and the run's outcome is answered as it is. When its agent published with `--merge`, the merge stays held.
- Otherwise a fresh run starts in the same process: a new id minted from the clock's now, its own marker, its own record and its own lock, a checkout named after its own id and attached to the first run's branch (the branch read back at the first run's end), and the prompt `<follow-up prompt> <first run's id>` (`/post-merge-cleanup 2026-09-16T14-01-00-000Z`). Its command is read off that prompt like any person's run (`post-merge-cleanup`). It runs on the coding agent made for its own id (Claude Code or Codex, the same one as the first run's, `scheduler.ts`), or on the first run's driver when none is made per id; it gets the first run's model, host, pid and clock. It names no follow-up of its own, so its checkout is not held and nothing follows it.
- When the follow-up's run ends `done`, the first run's held merge is released for the first run's pull request number (`branches`' release: the held line leaves the request's body and the merge is armed as `publish --merge` arms it), and the release's outcome (`auto-armed`, `merged`, `watching`, `failed`, `not-held` when the first agent never asked for the merge, `closed`) is answered with the follow-up's outcome.
- When it ends any other way, nothing is released: the merge stays held and the request open, with its held line, for a person.
