...
- **A schedule line** - `- work-queue: when \`npx queue\`, cap 2` reads as the command, its check and its cap with its line number, running where nobody switched it; a line with no cap reads as cap 1; headings and prose are not read.
