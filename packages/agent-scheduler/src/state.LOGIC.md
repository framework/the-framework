...
**User story**: the user turns the scheduler on and off, picks the model and the spend cushion for their own machine, turns a scheduled command on or off for their own machine with its schedule switch [4], and reads what the last tick [3] decided per command, in one file a dashboard can show as it is; a machine restart, or a crashed scheduler, leaves all of it in place.
...
- **A schedule switch per machine** - a command's schedule switch [4] is kept only where it differs from its schedule line, and switching it back to what the line says removes it; a command nobody switched runs as its line says.
