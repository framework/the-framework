Keeps a project's `.branches/` directory navigable by branch name; a daemon runs the reconcile on its clock and after each checkout it creates. Every checkout there is a directory named as the branch it was created on (`agent-<agent id>`), but an agent renames its branch to `agent-<session name>` early — so this pass maintains a symlink named as the branch each checkout is on *now*, whenever that differs from the directory's own name. `cd .branches/<name>` then reaches any agent's checkout by its name, and a rename costs a link — a checkout is never moved under a live agent.
