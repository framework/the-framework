...
- The branch is renamed to the session name only while the checkout is still on its original branch; an agent that already made its own branch is left alone, and a rename that fails never brings the agent down. (Naming as the agent asks for it, with its suffix and refusal rules, is covered by the command-line tests.)
- Listing the checkouts on disk names the agent of each `tf-agent-<agent id>` directory, ignores anything else in the branches directory, yields nothing for a project that never ran an agent, and never mistakes a rename link for a checkout.
