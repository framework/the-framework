The half of the Claude web bridge that talks to the daemon: it holds the daemon token, reports the question a cloud session is parked on to `POST /_bridge/question`, collects the answer the dashboard queued in reply and hands its text to the claude.ai tab, and keeps a pinned background tab open for every cloud session the daemon says is worth watching.
...
- **The token never enters the page** - every daemon call is made by the service worker; the content script only passes it what it scraped and receives back only the text to type.
...
- **The answer is typed before it is acknowledged** - it is handed to the tab first and only reported as delivered afterwards, so the dashboard never shows a session as answered when nothing was typed.
...
Every call to the daemon — reporting a question, sending transcript entries, saying hello, fetching an answer, acknowledging a delivery, listing watched sessions — is made by the service worker, which reads the daemon URL and token from the extension's own storage and sends `Authorization: Bearer <daemon token>`. The content script is only ever handed the text of an answer to type, and only ever passes back what it scraped from the page. Without a token stored, every path stops immediately and reports "no token set: open the extension options" rather than failing silently. The daemon URL defaults to `http://localhost:4200` when the user has not set one, and any trailing slashes on it are ignored.
...
Immediately after a question is accepted, that session is checked for a queued answer: the moment the page changed is also the moment a parked session is most likely to have just been dealt with.
...
Twice a minute, and again right after any question is reported, the service worker asks the daemon which cloud sessions it is watching and fetches `GET /_bridge/answer` for each. An answer comes back as a delivery id and the text to type — the daemon composes that text from the option the user picked, so the worker neither writes nor edits any of it. The worker finds the open tab whose URL carries that session's id, hands the text to that tab's content script to type and submit, and only then posts the outcome to `POST /_bridge/answered`, quoting the delivery id and whether it succeeded, plus the content script's note on failure. An answer without a delivery id or without text is ignored.

If no tab is showing that session, nothing happens and the answer stays queued for when one exists. An answer already handed to a tab is not handed to another one for as long as the service worker lives; a delivery that came back a failure releases that claim, so a page that recovers can be tried again.
...
Reloading the extension leaves the already-injected content scripts orphaned; Chrome also discards idle background tabs. Either way the tab showing the session stops answering, and the user's answer has nowhere to go.
...
An acknowledgement the daemon refuses for a transport reason is kept and re-sent at the start of every answer poll, until the daemon either accepts it or rejects it as malformed — a rejection is treated as settled, because retrying it would never succeed.
...
Both recurring jobs run off browser alarms rather than in-process timers, and both also run once when the service worker starts. Sessions are swept once a minute; answers are polled twice a minute, on their own faster beat because a person is watching a spinner at the other end of one.
