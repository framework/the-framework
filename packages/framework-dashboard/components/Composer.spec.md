...
- Modes: `compact` (#723/#755) is the one-row navbar quick-launch keeping agent/model + gear (their earlier omission meant navbar runs silently used stored options); `showAgentModel=false` (#831) for in-session (a session is bound to its start agent); `inSession` (#833) drops the "In play" strip and, while the run is LIVE, the gear entirely (#1172 — it used to render with an empty dropdown; nothing is adjustable mid-run). With `sessionEnded` the gear returns as "Resume options": the next message is a new leg that resolves current preferences at start (#1469), so it offers `resumeOptionRows` — the publish ladder, Autopilot, Browser — and nothing prompt-shaping (the resumed transcript carries its framing) nor "Run on"/agent/model (pinned by the conversation).
