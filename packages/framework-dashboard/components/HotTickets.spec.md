...
- `TicketTag`: `implementing` (coloured) when a run is on it, else `planned`/`spiked` for in-progress rows, else the priority for high-priority rows; AI-Queue rows carry nothing — the lane already says it.
...
- `implementing` is coloured while `planned`/`spiked` stay muted (#1117): it is the only tag describing something happening as you read it; the others are marks older work left behind.
