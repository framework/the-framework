...
- Bonus 2: a question answered HERE collapses to a single ✓ line (the shared `AnsweredChoice` card, with the session label as `meta` and the Open-session link as `footer`) and stays — expanding shows the options with the pick marked. The memory is per-mount (`answered` map keyed like the cards), fed by `ChoicePanel.onAnswered`; answered gates the poll still carries render collapsed in place, ones the poll dropped append after the open cards. A reload starts clean.
