...
//
// Shown as "History" (#1145): the file is a log, but the panel is the project's session history.
export function ProjectLogPanel({ logs, loaded }: { logs: LogEntry[]; loaded: boolean }) {
  ...
  if (logs.length === 0) {
    return <p className="p-4 text-sm text-muted-foreground">No finished sessions recorded in this project yet.</p>
  }

  return (
    <div className="flex min-h-0 flex-auto flex-col">
      <p className="shrink-0 border-b border-border px-4 py-2 text-xs text-muted-foreground">
        Finished sessions in this project, newest first — from <code className="font-mono">.the-framework/LOGS.md</code>,
        committed to the repo, so it survives a fresh clone.
      </p>
