...
- `HandoffActions`: the post-settle button — a single "Open PR" (opens the PR, pushing on the way); every no-button state says why instead (`Reason`): branch gone, nothing committed, no remote (#1173).
