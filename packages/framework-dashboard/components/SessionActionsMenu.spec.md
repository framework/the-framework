The ⋮ overflow menu for everything you can DO to a session (#toolbar-menu): GitHub, folder, editor, session link, resume-command copy, Serve, Stop, Remove worktree, Delete.
...
- Stop stays "Stopping…" (`stopRequested && active`) until the end event flips `active`, so it can't be re-fired; reset when `runId` changes.
