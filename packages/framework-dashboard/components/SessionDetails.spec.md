...
- Facts row: Agent (via `agentForDriver`/`AGENT_LABELS`, falling back to the raw driver name), Spent ($ when `costUsd` reported), Tokens in/out, Cache read/write (only when non-zero), Turns — or "No spend reported yet".
