The cross-project Tickets view (#1144): every registered project's `tickets/*.md` backlog, one section per project, with a sort dropdown — its own full page, not a right-rail tab.
...
- No status filter (the #1230 Open/Closed toggles are retired with the `status` key): a closed ticket is removed from the repo per the format, so everything listed is open work.
