...
- `onOpenTicket(projectId, file)` routes to the detail page; `onRunStarted` is re-bound per section with its own projectId, since which project started an import isn't implied the way it is on a single-project page (#948).
