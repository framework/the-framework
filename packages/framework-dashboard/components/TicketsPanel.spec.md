...
- Plan column (#685), a fixed cell past the date: `planned` → a `FileText` link that opens the plan view via `onOpenPlan(file)` (the `<icon-plan>`); not planned → a `Zap` button (`<icon-spike>`) that starts a session with `planPrompt(file)` = `Create tickets/<stem>.plan.md`. It replaces the old "planned" badge — the badge said the state, the column says it *and* gives the reader somewhere to go with it. `onOpenPlan` is optional; without it the link renders disabled.
