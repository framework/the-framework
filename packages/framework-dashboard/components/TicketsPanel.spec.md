...
- Plan column (#685), a fixed cell past the date, a clipboard pair so both states read as "a plan": `planned` → a `ClipboardList` link in strong `text-info` (blue) that opens the plan view via `onOpenPlan(file)`; not planned → a `ClipboardPlus` button in `text-muted-foreground/70` (light grey, the row's "4d ago" tone) that starts a session with `planPrompt(file)` = `Create tickets/<stem>.plan.md`. Chromatic-vs-grey, not just the glyph, tells the two near-identical clipboards apart (blue = a plan that exists and pops, grey = a recessive create action) and lets the column be scanned for what's planned. (`info` not `primary` for the blue: the theme defines `--primary` as the same colour as `--success`.) It replaces the old "planned" badge — the badge said the state, the column says it *and* gives the reader somewhere to go with it. `onOpenPlan` is optional; without it the link renders disabled.
