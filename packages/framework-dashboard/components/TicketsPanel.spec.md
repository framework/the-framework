...
- Plan column (#685), a fixed cell past the date, a clipboard pair so both states read as "a plan": `planned` → a `ClipboardList` link in `text-success` that opens the plan view via `onOpenPlan(file)`; not planned → a `ClipboardPlus` button in `text-primary` that starts a session with `planPrompt(file)` = `Create tickets/<stem>.plan.md`. Colour, not just the glyph, tells the two near-identical clipboards apart (green = a plan that's ready, accent = an action to take) and lets the column be scanned for what's planned. It replaces the old "planned" badge — the badge said the state, the column says it *and* gives the reader somewhere to go with it. `onOpenPlan` is optional; without it the link renders disabled.
