...
- Tickets (#1144): `TicketsPage` (cross-project sections or one flat list, URL-carried filter/sort state) + `TicketFilterBar` (search, faceted filters, sort/group menu) → `TicketsPanel` (one-liner `TicketRow`s + Import/Update-from-GitHub presets) → `TicketDetailPage` (full markdown + Queue).
