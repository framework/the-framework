...
- `isPublishing` (#1431) — the run ended clean but its armed handoff has not reported back: open after the CURRENT segment's clean `end`, closed by its `handoff` event (done, skipped, and failed all report). The closing check stays inside the segment — a resumed session's earlier `handoff` must not hide the new window (#1450) — while arming folds the whole feed (latest `handoff-armed` wins; run-level config, not segment state). Requires a real arming event with `push` on: absent-means-armed defaults would leave pre-handoff archives "publishing…" for ever.
