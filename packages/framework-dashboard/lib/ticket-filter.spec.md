...
- `sortRows(rows, {key, dir})`: date/priority/title/effort with direction; **a missing value sorts last in both directions** (Priority ↑ must not bury the list under unprioritized tickets); ties fall back to newest-first. `sortByKey` starts a freshly picked key at its natural direction (`DEFAULT_DIR`); direction changes are the toolbar's explicit asc/desc pair.
- `bucketUnionRange(filter, buckets)`: the one contiguous span a bucket selection covers (adjacent spans merge), or null when it skips a middle bucket — what lets the slider mirror the buckets when it can and dim when it cannot.
