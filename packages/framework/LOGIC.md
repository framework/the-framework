...
**Business logic story**: the package is a daemon and a browser app, and neither of them runs a coding agent [12]. Pressing Start runs one shell line the project itself names; whatever tool that line names begins the agent [3] and owns it. That tool keeps the agent's card [7] and diary [8] in the agent's checkout [6] and records both when the agent ends (the `logs` skill keeps them on the `agent-data` branch [9]). The daemon reads the checkout's files to show a working agent, and a finished one through the project's runs provider [14], and writes into the agent's inbox [10] — or runs the project's resume hook [11] — when the user says something to it. Because the record is the seam, watching an agent live and reading it back months later are the same projection of the same lines, and The Framework never learns which tool did the work.
...
[14] runs provider: the command, among the commands of a project's dependencies, that a package declares in its own `package.json` (`"framework": { "runs": "<command>" }`) as answering for the project's finished agents: it lists them, shows one with its whole diary, deletes one, and sets its branch and pull request. The Framework owns the shape of what it answers — the card [7] and the diary [8] — and nothing else about it.
...
- **Finished agents come from a package the project picks** - The Framework imports no records package: a finished agent [3] is whatever the project's runs provider [14] answers, read by running that command; no dependency provides, the dashboard shows only the agents that still have a checkout, and a finished agent's page says it is kept only when a logs skill is installed.
...
So a working agent and a finished one render identically, and an agent whose machine is elsewhere needs only its lines carried home. The lasting record outlives the checkout: the agent's tool records its card and diary when the agent ends (with the `logs` skill, onto the project's `agent-data` branch [9], which is also how a second machine can list what this one did), and the daemon reads the finished agent back through the project's runs provider [14]: the command a package of the project declares, run with this machine's copy and no network, every read shared for five seconds (`src/store/runs.ts`). The Framework defines the card and diary it expects and nothing about where they are kept, so a project may swap the package for any other that answers the same command line. With no runs provider the dashboard shows the agents that still have a checkout, and a finished agent's page says there is no record of it, since finished agents are kept only when the project has a logs skill installed.
