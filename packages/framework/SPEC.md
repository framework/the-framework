...
- `src/` — everything that runs in Node: the CLI, the daemon, the agent lifecycle (worktrees, gates, handoff), the driver seam that wraps each coding-agent CLI, the data branch, autonomy (Auto PM, sweeps, CI watch), and the server side of the dashboard.
