...
[9] location: where an agent's turns ran, as its own record names it: `local` (this machine), `actions` (a GitHub Actions runner), or `web` (a Claude Code cloud session). Only `local` and a device are offered today; the other two are read off agents recorded before they left the launcher.
...
[11] device: another machine's daemon the user saved by URL and token, to run agents on it from this dashboard.
...
[14] message: the user's own words to an agent, the next prompt of the same conversation: an agent that is working takes it when its turn ends, an ended agent is resumed with it.
[15] archive: the record of an agent that the tool running it keeps on the data branch, its status and its events, written when the agent ends.
...
- **The selected agent's live stream** - one live stream follows the agent in the URL and feeds the agent view; a new start empties it, a continuation keeps it.
...
- `/{project}/{agent id}` is the agent view [6] of that agent [1], running or finished; the second segment is the agent id [8].
...
**User story**: whatever page is open, the user keeps the sidebar on the left: "New", "Overview", "Tickets", the projects picker, the "Recent agents" list and, in its footer, which daemon the dashboard is talking to, the theme, notifications and "Settings". While a project is selected, the rail on the right offers the "Files" and "Docs" tabs.
...
**User story**: the user follows a link. A link to an agent [1] whose checkout [10] was removed, or to a project dropped from the registry, says so and offers the way back, rather than silently landing somewhere else as if the link had worked.
...
7. With no agent selected: the project home [5], handed the live events, the files and what the daemon currently finds wrong with the project, for its banner.
...
9. The agent view of the listed agent: live exactly while its status is `running`; labeled by what the user typed, else its session name [13], else its branch, else its start time (the rule in `lib/agent-label.ts`); told its location [9] and the device it runs on when relayed. A running and a finished agent get the same page, so an agent ending changes what its bar, feed and composer say without replacing the page. Deleting the agent from its page returns to the project home and reloads the agents list so its row is gone.
...
- A start reports the project it started in (not always the selected one), the prompt that was typed, the agent id [8] the project's start hook answered, and, for an agent relayed [12] to a device [11], that device's label.
...
- The live feed on screen is emptied for a new agent, but not for a continuation: when the reported id is the agent already on screen in the same project, which is a message [14] resuming an ended agent, the transcript keeps its history and the new turn appends to it.
...
- The selected project's agents, live and archived [15], every 2 seconds (`lib/use-agents.ts`): the sidebar's rows and the main pane's routing read one list.
- The selected project's files every 10 seconds, scoped to the selected agent's checkout [10] when an agent is selected (the same checkout its branch, preview and open-folder actions act on), so a file the agent creates shows up without a reload; empty when no project is selected.
...
**Business logic story**: an agent's [1] events reach the browser as they are written, over one live stream per selected agent. The agent view's [6] transcript reads it, and the one subscription is held here.

#### Business logic

- The stream follows the agent in the URL. With a project selected and no agent, it follows the project's own events, which is where an agent without a checkout [10] writes. With no project selected there is no stream.
...
- Two feeds notify: the interventions [7] and the activity feed. Each fires only when its category is on in the preferences [16] and browser delivery is on. The defaults are on for browser delivery and for the interventions' category, off for "New activity"; they are the daemon's own defaults (`src/preference-defaults.ts`), so the page and the daemon's Discord watcher agree. The browser's own notification permission is the last gate, checked in `lib/use-notifications.ts`.
