...
9. The agent view of the listed agent: live exactly while its status is `running`; labeled by what the user typed, else its session name [14], else its branch, else its start time (the rule in `lib/agent-label.ts`); told its location [10] and the device it runs on when relayed. A running and a finished agent get the same page, so an agent ending changes what its bar, feed and composer say without replacing the page. Deleting the agent from its page returns to the project home and reloads the agents list so its row is gone.
