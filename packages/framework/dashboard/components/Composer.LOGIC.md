...
- "Run on" lists the three locations [5] as "This machine", "GitHub Actions" and "Claude web" (the task is handed to a cloud session [12] that opens its own pull request), then every saved device [6] with an online/offline dot and a remove cross, then "Add a device…". The location is a preference [7]. The selected device is not: it is held in memory for the life of the page, because a device's token is a per-browser secret that never reaches the daemon's registry.
