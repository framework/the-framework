...
- **"Populate tickets/"** - on the board only while some registered project has a package providing tickets; optional; done once any project has tickets; "Update from GitHub" starts an agent [1] with the project's `update-tickets` command on the target project and lands the user on it, or, through its chevron, opens that project's launcher [6] with the prompt.
