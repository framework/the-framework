...
- **routine lock** — a routine's lock file on the `agent-data` branch, naming the machine running that routine and since when, so no two machines run it at once. A routine declares whether it holds one.
