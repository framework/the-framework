...
Whether the text is a command, a saved prompt [8] or the user's own words makes no difference to what is sent: it is one prompt.
...
The start hook answers the new agent's id. The form hands the shell the typed text, that id and, for a relayed [7] agent, the device's label: the shell selects exactly that agent and shows it at once under the typed prompt until the agent's own files take over, marked with the device it runs on. The editor is then emptied. A refused start leaves the text in the editor.
