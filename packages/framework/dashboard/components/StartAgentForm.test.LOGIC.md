...
- **No start hook** - a project read as having no start hook shows the alert naming `npx agent-runner init`, the `start:` line and `.the-framework/hooks.yml`, and the submit stays disabled with text in the editor.
