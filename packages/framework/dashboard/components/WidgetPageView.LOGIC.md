Renders one widget's [1] page in the dashboard's main pane, through the slot every widget piece renders in: the host the piece reads, and the boundary that keeps its failure its own. The Overview's cards render through the same slot (`WidgetCards.tsx`).
