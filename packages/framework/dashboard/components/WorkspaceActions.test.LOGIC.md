...
- **The same offer on both pages** - a project's page and an agent's page carry the same set of actions and the same repository link (the forge's page, labelled with its name), so an agent's page is not missing the repository, the folder or the editor.
