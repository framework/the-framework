...
// shadcn's "base" (Base UI, not Radix) Sidebar, ported to this repo (#314 rail, shared shell). The
// only deviations from upstream: it reads this repo's `cn`/`Button`/`Tooltip`; the collapsed-state
...
function SidebarProvider({
  ...
  // Internal open state; open/onOpenChange let a host control it from outside.
  ...
  // A data-state to drive the Tailwind styling (`data-state="expanded" | "collapsed"`).
...
function SidebarMenuSkeleton({
  ...
  // A stable pseudo-random width per row, seeded off React's id rather than Math.random so it does
  // not churn on every render (upstream re-rolls on mount; this is steady).
  const rid = React.useId()
  const width = `${50 + (rid.split('').reduce((a, c) => a + c.charCodeAt(0), 0) % 40)}%`
