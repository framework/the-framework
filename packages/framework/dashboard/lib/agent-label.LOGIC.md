...
[3] agent id: an agent's stable id, derived from the moment it started; it names the agent's checkout directory, its branch until the agent names it, and its run.
...
- **Otherwise its session name** - the name the agent gave its own work, which is its branch minus the `agent-` prefix. A branch that is still the one the checkout was created on, named after the agent id [3], is not a name the agent chose and does not count (the rule is in `branch-names.ts` of the `branches` skill).
- **Otherwise its branch** - a branch that carries no session name is shown as it is, with surrounding whitespace removed.
- **Otherwise when it started** - an agent that nothing else describes is labelled with its start time, in the dashboard's short date and time wording (the rules are in `format-date.ts`). A date is a poor name but a real one, and it belongs on the line that identifies the row.
