...
- **"ready for merge"** - an agent that signalled ready for merge and then ended clean shows "ready for merge".
- **The ending outranks the signal** - an agent that signalled ready for merge and was then stopped shows "stopped"; one that signalled it and then failed shows "failed" with the reason its ending carried appended, as "failed — exit 1".
