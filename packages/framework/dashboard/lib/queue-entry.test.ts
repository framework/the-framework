...
describe('queueEntryLabel (#1164)', () => {
  test('a queued ticket reads as its title, and carries the path its link points at', () => {
    // What the Overview card actually showed: `[Improve tooltip: show it with no...`, truncated
    // mid-source, which is why the queue looked like it had not worked. The path is what the
    // link points at, kept whole: which page opens it is the mounted widgets' to say (#1774).
    const entry = '[Improve tooltip](tickets/2026-07-25_improve-tooltip.md)'
    expect(queueEntryLabel(entry)).toEqual({ text: 'Improve tooltip', path: 'tickets/2026-07-25_improve-tooltip.md' })
  })
  ...
  test('a link to any other repository path keeps its title and the path', () => {
    // Whether a page opens it is decided where the label is shown; the label only carries it.
    expect(queueEntryLabel('[Read the docs](README.md)')).toEqual({ text: 'Read the docs', path: 'README.md' })
  })
