...
// a persisted preference. Null means this machine: the run starts here, through this project's own
// start hook. Node-free leaf like profiles.ts, so nothing leaks into the SPA bundle. The value is
// the device's profile id (its origin url).

let selectedDeviceId: string | null = null
const listeners = new Set<() => void>()

/** Select a saved device (by profile id) as the run's target, or null to go back to this machine. */
