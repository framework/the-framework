...
- **Starting an agent** - start a build agent [9], a prompt agent or a research agent [10] with the launcher's [11] options, and learn whether it started (with its agent id [12] when it got a checkout [13] of its own), was refused because the same work is already active, or failed and why.
...
- **Tickets and the agent queue** - put a ticket, or a ticket's plan, on the agent queue [14] in the section its priority earns, and release the claim [15] an agent left on a ticket.
