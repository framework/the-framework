...
- **The registered projects** - every project the registry lists, with its id, path, name, whether it is still activated, its last activity, the defaults its repo file [4] sets, and whatever the daemon's sweeps [3] currently find wrong with it, which rides on this list because every project surface already polls it.
