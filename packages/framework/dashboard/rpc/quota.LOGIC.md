...
- **Run the sweep now** - an Auto PM sweep fired on demand and awaited, answering its outcomes one line per project, or that the sweep itself failed. The Auto PM preference does not gate it: that preference is consent to spend quota unasked, and this call is asking, so the sweep runs once even with Auto PM off, every other stand-down reason still in force. It can be narrowed to one routine's [8] work, `drain` [5] fanning out [7] one agent per entry of the agent queue [6], `plan` one per open ticket, or the routine pinned to one branch, and to one project.
