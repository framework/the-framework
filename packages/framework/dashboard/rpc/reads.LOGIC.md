...
[7] open question: A gate nobody has answered yet, as the dashboard lists them across projects.
...
- **The agent history and one agent** - a project's agents [1] newest first, the ids of ended agents whose checkout [3] is still on disk, where one agent is working (its checkout, branch, uncommitted changes, size and pull request), and one agent's replay: its recorded events.
...
- **Cross-project rollups** - every registered project's tickets, the aggregated agent queue [4], the Overview [5], the recent agents, the hot tickets, the interventions [6], the open questions [7], the activity feed and the dashboard page, each read across every project the registry lists.
...
- **Git and the handoff** - the project's GitHub URL, the git status (branch, uncommitted changes, linked pull request) of the project or of one agent's checkout, and what an agent's handoff [8] left behind: its branch, commits, changes, pull request and uncommitted work.
...
- **The bridge's state** - the question a cloud session [9] is parked on, where the pick [10] the user made stands, what the session has said so far, whether the extension has reached the daemon and how it went, the bridge token, and the bridge browser's state.
