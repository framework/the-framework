...
- **Cross-project rollups** - the aggregated agent queue [5], the Overview [6], the recent agents, the Overview's "hot tickets", the interventions [7], the open questions [8], the activity feed, the dashboard page's totals, agents going right now, projects and queues, and every project's scheduler state, each read across every registered project.
