...
- **Tickets and the agent queue** - the section following it, for the same agent: the fallback that has it read and write the `tickets` branch with git itself, followed by the `tickets` skill's own text for the formats. Temporary. An agent in a checkout The Framework created has the `tickets` skill in the checkout instead.
...
- **Branch management, and its counterpart for the tickets** — two sections appended after the built-in prompt for an agent that runs outside a checkout The Framework created, where neither of the two commands exists: one has the agent create its branch with git itself, the other has it read the tickets and the agent queue off the `tickets` branch and write them as commits pushed straight to that branch, never onto its own. The second is followed by the `tickets` skill's own text, so the ticket, plan, claim and queue formats reach the agent in the one place they are written. Neither is sent to an agent that finds the skills in its own checkout. The ticket half is temporary even as a fallback: it goes as soon as the skill is committed into the repository, where every agent finds it.
