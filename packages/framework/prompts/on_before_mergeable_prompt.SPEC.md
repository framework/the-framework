...
- **Quality work is queued, not run** - the agent puts entries on the agent queue (`TODO_AGENTS.md`), with the `queue` skill's command, asking for the quality presets to be applied to its own changes; a later drain does the work.
