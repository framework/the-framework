The "Suggest tickets to work on" preset of the launcher, the gated sibling of the two triage presets: the agent [1] looks at all tickets, picks the ones to work on next, shows them as a multi-select with its confident picks checked, stops at a gate [2] for the user's approval, and puts each approved ticket on the agent queue [3] at the ticket's own priority. Its tooltip reads "Add tickets to queue (TODO_AGENTS.md)". It takes no parameter. Because it ends in a gate, it is not to be run unattended [5]: an agent nobody answers would be wedged against a human who is not there.
