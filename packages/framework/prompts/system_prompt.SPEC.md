...
When the scope is *potentially very large* — spanning many hours or days — the agent additionally considers adding follow-up tasks to the agent queue (`TODO_AGENTS.md`) and showing the new entries as a markdown view, so the overflow becomes queued work rather than being lost. The prompt says how such an entry is added: with the `queue` skill's command, which puts it on the queue at the priority the agent gives it.
