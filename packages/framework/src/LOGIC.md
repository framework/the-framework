...
- **Spending** (`quota-boundary.ts`, `quota-poller.ts`, `auto-pm.ts`, `daemon-writes.ts`, `maintenance.ts`, `routine-locks.ts`, `handoff-level.ts`, `on-before-mergeable-prompt.ts`) - the share of the quota [6] week that may be spent by now, what the daemon starts under it when the `agent-data` branch moves, the trailer that tells the daemon's own commits from a move, the routines [15] it rotates through, the lock that keeps a routine to one machine, and the ladder a finished agent publishes itself by.
