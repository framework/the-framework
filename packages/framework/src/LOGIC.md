...
- **Starting and steering an agent** (`project-hooks.ts`, `project-commands.ts`, `open-choices.ts`) - the five shell lines a project may name for itself (open, close, start [3], resume [9], offset, which sets the spend offset [15] of the project's scheduler), the project's commands [4] read off its skills folders, and the one rule for which question an agent is still waiting on.
