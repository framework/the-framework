...
- **The data branch** - tickets, the agent queue, and agent archives are committed to the `agents-data` branch through one serialized sync → apply → commit → push funnel, giving every machine and cloud session the same view while the default branch stays 100% code.
