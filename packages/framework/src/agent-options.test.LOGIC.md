...
- **The repo file sits over the user's tier, key by key** - a key the file leaves unset keeps the user's answer; an explicit `false` in the file wins over the user's `true`, not only a `true`; merging the two tiers and then mapping, the path the daemon's own starts take, lets the file's transparent win while the user's model stands untouched.
