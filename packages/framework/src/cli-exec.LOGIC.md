...
**User story**: the user presses a button in the dashboard that pushes a branch or opens a pull request. When the `gh` command is not installed, is not logged in, or the repository has no default remote, what the user reads is what `gh` itself said ("not logged in", "no default remote") rather than a generic "command failed"; and a command that never answers gives up on its own instead of leaving the user waiting.
