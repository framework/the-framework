...
- **What the agent's record learns at start** - the armed handoff as three stages and the branch it starts on are emitted before the first turn.
...
**Problem**: the control file [5] carries instructions, but only an event reaches the agent's record, and the record is the only thing a dashboard tab opened mid-way can read the checkboxes and the branch back from.
