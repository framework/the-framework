...
- **The renamed branch.** The agent renames its branch itself, through `branch-management name` (#1725), invisibly to the runner until it looks: the branch is re-read at the end of every turn and once more before the ending — ahead of the quality step, the handoff and its merge gate, which all read the same observation — and a change is recorded. A checkout that left its branch (a detached HEAD) is remembered as having none, so the handoff skips rather than publishing the last name seen. The session name is the recorded branch minus its `tf-` prefix — none while the branch is still the birth branch `tf-agent-<agent id>`.
