...
- **How an agent's activity reads** - an agent [3]'s events [4] rendered as terminal text, which options a pick [5] chose, the driver session [6] behind the agent, the agent's progress, the errors it reported, and where its handoff [2] stands.
