...
[7] event: everything an agent does, one event per line appended to its event stream; every surface is a projection of it.
...
[10] spend offset: the user's adjustment of the quota boundary — the share of the account's subscription allowance that may be spent by now — in percentage points of the week.
...
[13] gate: a question with options at which an agent stops and waits for an answer.
...
- **How an agent's activity reads** - an agent [5]'s events [7] rendered as terminal text, which options a pick [8] chose, the driver session [9] behind the agent, the agent's progress, the errors it reported, and where its handoff [4] stands.
...
- **The defaults and limits the user adjusts** - the notification defaults, whether a notification method and category is on, and the default and maximum spend offset [10].
...
- **Rules about the cloud side** - a question the Claude web bridge [12] holds rendered as a gate [13], and the word for what a web agent [5]'s cloud side is doing, so every surface derives the same one.
- **Small shared renderings** - a driver [14]'s name and label, a byte count as a short label, and an error rendered as a message.
