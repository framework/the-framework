...
- **A real sentence keeps its words** - a plan's closing sentence ("…then comment on and close #42") comes out reading the same, with only "the ticket" inserted before the reference, so a reviewer sees prose rather than an escape.
...
- **The colon form** - a keyword joined to the reference by a colon (`Fixes: #42`, `CLOSES: #10`, `resolves:#7`) closes just as well and is defused too, with the colon and its spacing kept exactly as written.
