Decides the one word every surface shows for what a web agent [1]'s cloud side is doing — "waiting", "in cloud" or "done" — from facts the daemon already holds about the agent: whether the Claude web bridge [2] holds a gate [3] its cloud session [4] is parked on, whether it has a pull request, and how long ago it started. Every other agent gets no cloud word, and its own status is what its row says.

## Context

**User story**: the user has several web agents [1] on record and watches the dashboard's agent list and the Overview [5]. The dashboard starts no web agent today; the word stays for the records of web agents and for web agents' return. An agent whose cloud session [4] is still working reads "in cloud", one whose session stopped to ask something reads "waiting", and one that is over reads "done" like any other finished agent.

**Problem**: a web agent's local half ends the moment it hands the task to a cloud session, so from then on its stored status is "done" — which says nothing about whether the session is still working on claude.ai, is parked on a question, or finished long ago. Marking every such agent "in cloud" forever is the opposite error: an agent that finished two days ago is not in the cloud. The word has to come from what is actually known about the session.
...
[1] agent: the unit of work: one task worked by a coding agent in its own checkout, on its own branch, started through the project's start hook and shown in the dashboard from the files its tool keeps. A web agent is one whose location is `web`.
...
- **What still counts as an agent at work** - "waiting" and "in cloud" mean the cloud side is still working or still owed an answer; "done" means it is over.
...
Once a pull request is recorded on the agent [1] — the cloud session [4]'s own, or the one cloud work adoption [8] opened — the word is "done": the work left the session and landed on a branch, exactly as a finished local agent with a pull request reads. What is happening to that pull request is the pull request badge's job to say.
...
With no pull request recorded, an agent [1] that started no more than 12 hours ago — the cloud session window [7] — reads "in cloud": its cloud session [4] may still be working. An agent older than that reads "done", since either the session finished without ever pushing or it never will. An agent whose recorded start time cannot be read also reads "done", so an unreadable record never claims work is in flight.
...
"waiting" and "in cloud" count as an agent [1] still at work: the cloud session [4] is working, or is owed an answer from a human. "done", and having no cloud word at all, do not.
