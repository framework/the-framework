...
- Only the driver's exact `cloud-<counter>-<8 hex>` naming and agent branches are candidates; a user's own `cloud-…` branch, `claude/*` branches, `agent-<session name>` branches, and agent branches whose age cannot be read from their name are never considered.
