...
For each waiting agent [5], the `claude/*` heads that contain the anchor commit are listed in one git question. Exactly one head is the agent's branch. None means the session has not pushed yet, or never will, and several is a history this sweep [2] cannot arbitrate; both are simply the next pass's question, without a word. An anchor whose commit is not local reads as no match, which is exactly the case where nothing has been pushed for it to be an ancestor of. An agent whose record already names another cloud branch, not this head, is skipped: a pull request opened here would be recorded against a branch it does not live on. A record naming a branch of the agent's own (one renamed by hand, say) is still on its birth branch for this purpose: the anchor's descent is the proof, and the record then names the branch the pull request lives on.
