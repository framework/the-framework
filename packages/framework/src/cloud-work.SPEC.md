...
- **What is learned lands as one commit** - the branch (first time only) and the PR (once known) are recorded onto the agent's run card on the data branch, through the `logs` skill; nothing learned, nothing written.
...
Whatever the pass learned — the branch on first adoption, the PR once known — is recorded onto the agent's run card as one commit on the data branch, through the `logs` skill. A record that cannot be written is a reported failure and is retried.
