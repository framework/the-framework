...
[6] hooks: the shell lines a project's own `.the-framework/hooks.yml` names: `open` and `close` lists run when the dashboard opens and closes, and the `start`, `resume` and `offset` lines.
...
- **Starting an agent** - the project's start hook [2] is run with the prompt and the user's picks; the id it answers is the Start's answer; a refusal is its words.
