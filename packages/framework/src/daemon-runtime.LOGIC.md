...
[6] hooks: the shell lines a project's own `.the-framework/hooks.yml` names: `open` and `close` lists run when the dashboard opens and closes, and the `start` and `resume` lines.
