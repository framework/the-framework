...
- **Retiring a finished agent** - its history is recorded on the data branch as the `logs` skill's run, then its checkout is removed once its work has reached the remote, and kept otherwise; any branch that went with it is named.
