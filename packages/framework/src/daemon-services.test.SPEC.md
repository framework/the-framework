...
Retiring a drained queue entry: once the agent's record says it finished and its handoff published the work, the daemon — the queue's only local writer — checks that entry off as a data branch commit, leaving every other entry exactly as written; the agent never touches the queue itself. An agent whose handoff failed leaves its entry open, so unpublished work is not retired.
...
Data branch sync: a project that cannot reach a remote carries a data-sync error naming the reason, and the daemon's log says so too; the first sync that converges clears the error rather than re-wording it.
