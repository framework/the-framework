...
Retiring a drained queue entry: once the agent's record says it finished and its handoff published the work, the daemon — the queue's only local writer — deletes that entry as a commit on the `tickets` branch, leaving every other entry exactly as written; the agent never touches the queue itself. An agent whose handoff failed leaves its entry open, so unpublished work is not retired.
