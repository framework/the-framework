...
Finished agents' archives need no flushing step here: each one is committed and pushed through the data branch's write cycle, as the `logs` skill's run, the moment that agent settles.
