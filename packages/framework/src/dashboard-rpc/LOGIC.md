...
- **Reading** (`reads.ts`, `reads.test.ts`) - everything the pages show about a project or an agent: its history, one agent's replay, the surfaced documents, the cross-project rollups, the changed files and their diffs, the branch's state and the handoff.
