...
- Removing a worktree that holds uncommitted work keeps it and says so — nothing is committed on the agent's behalf; a worktree whose agent committed is pushed to its remote and then removed, so the removed work stays recoverable.
