...
The project must be known here ("this project has no local path on this server"). The removal then runs under the agent's lock, keyed by the project and the agent's id, so two removals of the same checkout, or a removal and a pull request being opened from it, run one after the other. The checks and the removal are the shared implementation's (`../worktrees.ts`): an id unsafe for a path is refused before anything is touched ("invalid session id: …"); an agent still going is refused ("that session is still going; stop it before removing its worktree"); a project with no branches provider is refused ("no package of this project provides its checkouts"); the rest is the provider's `remove`, answered in its own words: an agent with no checkout is reported rather than claimed removed ("no checkout for agent …"); a checkout holding uncommitted work is kept and the edit stays in it, nothing is committed on the agent's behalf ("… has uncommitted work; the checkout was kept"); a branch the remote does not have is pushed first, and if it cannot be the checkout is kept ("… is not on the remote (…); the checkout was kept"). When the checks pass the checkout is removed, its committed work surviving on its branch and on the remote.
