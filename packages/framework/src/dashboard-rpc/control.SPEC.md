...
- **Finishing steps publish what the agent committed** - Push and Open PR push the agent's branch as the agent left it, committing nothing on its behalf; the pull request's title and body come from what the agent already recorded, and the opened pull request is written back onto the agent's archive.
...
- **Removing a checkout never loses work, and never surprises a running agent** - a checkout holding uncommitted work is kept and a clean one is pushed before the worktree goes, a still-running agent is refused, and anything serving that checkout is stopped first.
...
The push is performed while holding that checkout's lock. Clicked the instant an agent flips to finished, it otherwise races the daemon's own teardown pushing the same branch: two pushes creating the same remote branch, where the loser fails and the checkout is kept instead of retired.
