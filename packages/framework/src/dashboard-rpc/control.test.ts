...
test('sendReleaseTicketLock deletes the lock and commits the release (#1420/#1582)', async () => {
  ...
  try {
    ...
    const git = nodeGitRunner()
    await assert.rejects(git(['show', 'agents-data:tickets/2026-07-20_thing.lock.md'], cwd))
    assert.equal(await git(['show', 'agents-data:tickets/2026-07-20_thing.md'], cwd), '# Thing\n')
  } finally {
    await rm(cwd, { recursive: true, force: true })
  }
})
