...
- **Pull requests, once** (`pull-requests.ts`, `cache.ts`) - every pull request the product reads (a branch's, a branch's whole history, the project's open ones) comes through the forge provider a project's package declares (`../store/forge.ts`), in one place and in the product's own shape, behind a cache that collapses concurrent asks and serves a stale answer while it refreshes; the product runs no forge tool, and a project with no forge package has no pull requests.
