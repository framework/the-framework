...
- **What the pages read** (`dashboard.ts`, `projects.ts`, `overview.ts`, `open-questions.ts`, `queue.ts`, `scheduler-state.ts`, `tickets.ts`, `quota.ts`, `docs.ts`, `types.ts`) - the project list that survives a renamed directory, the cross-project Overview with its lanes, every unanswered gate [7] across projects, the agent queue [4] and the tickets as the browser needs them, each project's scheduler state as the scheduler's own file says it, the quota [5] panel, the documents shown beside an agent [3], and the vocabulary all of it is spoken in.
...
- **Getting the work out** (`agent-handoff.ts`) - what a finished agent left on its branch, and the "Open PR" and "Merge" buttons: opening the pull request, its branch pushed first, and merging it.
