Builds the activity [1] feed, the cross-project list of agents [2] that started or finished: one item per recent agent of every registered project, keyed so that each start and each finish is announced exactly once.

## Context

**User story**: with the "New activity" notification category switched on (it is off until the user turns it on), the user hears when an agent [2] kicks off and when it lands, as a browser notification, and hears nothing about what already existed when the page opened. It is the counterpart of the intervention [3] feed, which is always on and lists what needs a human.

**Business logic story**: the dashboard polls this feed for browser notifications, folds what it sees into a per-project baseline and announces only what appears after it, by the rule in `keyed-watcher.ts`.
...
[5] stop: ending an agent before it finishes: the Stop button or Ctrl-C.
...
**Problem**: "nothing happened there" and "I could not look" are the same empty list to everyone but the browser notifications, whose baseline must come from a real read: a first look that could not reach a project would otherwise make the next successful look announce everything pre-existing as new.
