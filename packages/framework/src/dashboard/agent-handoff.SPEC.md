...
The branch is the one recorded while the agent's worktree existed — authoritative because the agent renames its branch itself, which makes any derivation a guess. For an agent whose branch was never recorded (one started outside a git checkout) the birth spelling `tf-agent-<agent id>` is the one fallback.
