...
- **Where the files are read from** - the checkout the branches provider lists for the agent; else the branch the agent recorded, this machine's copy first, then origin's copy; else the commit the agent's recorded pull request merged as; else gone. Nothing is fetched and nothing is copied.
...
4. Gone. An agent that recorded no branch, or whose branch and merge commit are not on this machine, has nothing left to read.
...
The content: in a checkout, the file on disk (`file-read.ts`). On a branch or at a merge commit, the file as that commit holds it, capped at 500 lines, flagged binary when it holds a NUL byte, and nothing for an unsafe path or a path the commit does not hold.
