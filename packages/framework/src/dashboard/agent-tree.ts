...
// branch is gone too, the commit its pull request merged as. Every source is read through git by
// ref in the project's own repository, never copied, and never fetched: the tab polls, and a fetch
// on a poll is a network call. A run none of them is left for is gone, and says so.
...
export interface AgentFilesDeps {
  ...
  agent?: (root: string, agentId: string) => Promise<{ id: string; branch?: string; pr?: { number: number } } | undefined>
 ...
 * gone. A branch the default branch already contains (a true merge) shows no change, so there
...
export async function resolveAgentFiles(root: string, agentId: string, deps: AgentFilesDeps = {}): Promise<AgentFilesAt> {
  ...
  if (!agent || branch === undefined) return { source: 'gone' }
 ...
 * lists the commit's tree and marks what it changed, all committed.
 */
...
export async function readAgentFileDiff(root: string, at: AgentFilesAt, path: string, git: GitRunner = nodeGitRunner()): Promise<FileDiff | null> {
  if (at.source === 'pending' || at.source === 'gone' || !safeRepoPath(path)) return null
  if (at.source === 'checkout') {
