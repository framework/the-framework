...
- **The question's own shape travels whole** - a multi-select question, an option that starts checked, an option that means the user is taking over, and an option's detail line all arrive intact; a flag set to false and a key the daemon does not know are both dropped rather than carried through.
...
- **Validation** - a body that is not JSON, a body that is not an object, a malformed cloud session id, an empty title, an empty option list, an option without a label, more than twenty options, a recommendation naming no listed option, two options carrying the same label, and any of the three true/false flags given as something else are each refused with a message naming what was wrong; nothing is recorded from a refused call.
