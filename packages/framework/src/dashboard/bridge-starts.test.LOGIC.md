...
- **The branch must be a git branch name** - a name with a space, one carrying shell syntax or command substitution, an empty one and a 300-character one are refused; the ref an agent pushed for its cloud session passes.
- **The prompt** - a blank prompt and one over 200,000 characters are refused; a 50,000-character prompt, the size a cloud session's whole opening prompt reaches, is accepted.
