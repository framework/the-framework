...
- **A queued answer can be withdrawn, until it is delivered** - once a Driver has collected it, or the extension has delivered it, it is too late.
- **An answer is handed to one Driver at a time** - the user's own Chrome and the daemon's bridge browser can both serve a session; the first to ask gets the answer, the next asking within the claim's lifetime gets nothing, and a claim nobody acknowledged expires so the answer is offered again.
...
A waiting answer is handed to the Driver asking for it and marked as collected; another Driver asking within the claim's lifetime (a minute and a half) gets nothing, so a session served by two browsers — the user's own Chrome and the daemon's bridge browser — is not answered twice. A collected answer nobody reported on within that lifetime is presumed lost with its Driver and is offered again. A waiting answer can be withdrawn while no Driver holds it; one a Driver has collected, or the extension has already delivered, cannot.
