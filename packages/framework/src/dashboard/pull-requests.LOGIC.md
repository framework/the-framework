Every pull request the dashboard reads, in one place, all through the project's forge provider [1]: the pull request a branch has, a branch's whole pull request history, and the project's open pull requests. Reads only: opening and landing a pull request are the handoff's (`agent-handoff.ts`), through the same provider. The Framework runs no forge tool of its own; a project with no forge provider has no pull requests, and every read here answers "none" for it. It also owns the two shapes the rest of the dashboard speaks in, a linked pull request [2] and an open pull request [3], copied out of what the provider answers so nothing a provider adds leaks into what callers store, and the rule that says which of a branch's pull requests belongs to an agent [4].

## Context

**User story**: the user sees an agent's pull request in the git status bar and in the handoff summary, the project's open pull requests in the Human Queue, and the "Open PR" button only when the agent has none yet, whether the project is on GitHub or on another forge whose package answers the same command.
...
**Problem**: a branch name gets reused, so the newest pull request on a name may be a predecessor's; and "no pull requests" and "the forge could not answer" must not look alike to a caller about to open one.

## Glossary

[1] forge provider: the package of the project that declares it provides the forge (`"framework": { "forge": "<command>" }`); The Framework reads and moves pull requests through the command that package declares (`../store/forge.ts`). A project with none has no forge.
...
- **A branch's history** - every pull request the branch name has had, newest first, asked of the forge provider [1] for that branch in every state; the newest one is the branch's pull request; a project with no forge has none.
- **"None" against "could not tell"** - for the panels a forge that could not answer reads as no history; for a caller about to open a pull request it is a failure, never "none".
...
- **The open pull requests** - the project's, asked of the forge provider by state; a forge that could not answer is a failure, never an empty queue; a project with no forge has none open.
...
The forge provider [1] is asked for the project's open pull requests, which become open pull requests [3]. Unlike the panels' reads, a provider that could not answer (no remote, not logged in, the forge unreachable) is a failure the caller sees, never an empty list: "no pull requests are open" and "I could not look" are different answers, and taking the second for the first would make the next successful read announce every already-open pull request as new. A project with no forge provider has none open, and that is a real answer.
