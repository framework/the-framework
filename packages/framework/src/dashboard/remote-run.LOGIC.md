...
Registering a relayed agent, under the id the device gave it, opens a local live stream the dashboard reads through its normal same-origin events channel, and starts the pump that feeds it from the device. Registering the same id again replaces the earlier pump. Each event pumped in is also folded into a local list row for the agent, so the row mirrors what the device's own card of the agent says: the coding agent's session id once it is known, the cost added up, and the status when the agent ends (`done`, `stopped`, `failed`, or `waiting` when it ended on a question); because a relayed event carries no write time, the row is stamped with the local time of arrival. A project's rows are listed newest first. When the stream ends, it is closed cleanly, which the browser reads as "done", and a row still marked running is marked stopped, because no terminal event arrived and the agent is no longer live. The device the agent runs on stays known past the stream's end, so later calls still reach it, until the daemon shuts down: shutdown cancels every pump, closes every stream and forgets every device and every row. Nothing here is ever written to disk: the device's URL and token live in memory for the agent's lifetime.
