...
- **A start's options** - what a Start carries besides its prompt, each absent field leaving the choice to the tool the start hook [4] names: the model, and the driver [2], handed to the hook as `MODEL` and `DRIVER`; the follow-up's prompt, handed to the hook as `THEN` (once the agent ends done with a pull request, a fresh agent works its branch from it before the request merges: the launcher's "Post-merge cleanup" box); and the device [19] to run on, as its URL, its token and a label, which the dashboard sets at submit time from a saved device, is held in memory only, is never written to the preferences [21] or the registry, and is stripped before the start is forwarded so the device starts an ordinary local agent and never relays [20] onward.
