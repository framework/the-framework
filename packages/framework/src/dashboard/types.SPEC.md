...
- **A pre-minted identity** — the id this run is born with, when the caller minted it before the start: the daemon claims a ticket for the agent under that id and then starts the agent with it, so the claim and the run are one thing and a claimed ticket can name the session holding it. Left unset, a run's id is the moment it started.
