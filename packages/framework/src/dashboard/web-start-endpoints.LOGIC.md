The agent-facing side of the cloud session [1] start queue: how a web agent's [2] own process asks the daemon for a cloud session that the Chrome extension of the Claude web bridge [3] creates on claude.ai, and learns what became of the request. The agent is a separate process the daemon never started, so it cannot touch the daemon's queue directly: it reaches these routes at the daemon URL it finds in its environment as `TF_DAEMON_URL`, presenting the bridge token it reads from the registry. Nothing sets `TF_DAEMON_URL` today: web agents left the launcher when the daemon stopped running agents, so these routes have no caller until they return; the extension's side of the queue is live either way, which is why the routes stay. The extension's own side of the same queue is the bridge's start routes (`bridge-starts.ts`, `bridge-endpoints.ts`).

## Context

**User story**: an agent [2] with location `web` (none is started today; see above) needs a cloud session [1] opened on claude.ai for the repository, the branch and the prompt, which only the extension in the user's browser can do; its process queues the request here, then reads back the cloud session it became and its URL.
