...
- **A request is validated as the queue validates it** - repository, branch and prompt must be strings, so must the model when one is given, and the queue's own rules decide the rest.
...
A run posts the repository, the branch, the prompt, and the model it was started with when it has one. With the bridge off the route is not found; without the daemon token it is refused; when no extension has been let through the bridge within the presence window it is refused as a conflict naming that reason; a malformed request is refused with the queue's reason. Otherwise the request is queued and the run is handed its id.
