What the tests cover: driving the Codex CLI and translating a real Codex turn into the framework's event stream.
...
- This driver reports no quota at all, which means spending limits simply do not apply to a Codex agent rather than being applied to a made-up figure.
