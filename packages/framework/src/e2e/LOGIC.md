...
[4] card / diary: an agent's record in two shapes, defined by The Framework: the card `<id>.json` and the diary `<id>.jsonl`. While the agent has a checkout they sit under the checkout's `.the-framework/`, written by the tool that runs it; a finished agent's are recorded by the `logs` skill on the `agent-data` branch, and the dashboard reads them through the project's runs provider, which the fixture project's installed `logs` package is.
...
- **The simulated world** (`harness.ts`, `fake-run-bin.ts`) - one daemon runtime per story on throwaway state, wired as in production, with real git repositories as projects (each with a bare `origin`, its tickets and agent queue seeded on the `agent-data` branch [5], and a hooks file naming the stand-in tool), and agents that are real detached processes writing a real card and diary [4], reading a real inbox [7], recorded through the `logs` skill and reclaimed by the branches rule; the `logs`, `queue`, `tickets` and `branches` packages are each fixture's dependencies, so the runs, the queue, the tickets and the checkouts are read the way a real project's are, through the commands those packages declare.
...
- **Tickets and the queue** (`story-tickets-and-queue.test.ts`) - the ticket backlog read the way the tickets widget reads it, through the tickets package's command, and the way The Framework reads it, through the provider; a claim released by hand through the same command as an act, seen gone at once; and a queued ticket read through the project's queue provider so the boards show it queued.
