...
- The **branch** the agent's work is on, observed off the checkout: announced at start with the branch the agent actually begins on, and again whenever a later read — at the end of every turn, and before the ending — finds it changed, which is how the agent's own `branch-management name` reaches the record. The event also carries the session name the branch holds, when it holds one: the branch minus its `agent-` prefix, and none while the branch is still the one the checkout was created on (`agent-<agent id>`). The runner records the name because it alone knows that first branch; a reader of the stream cannot tell it from a named one.
