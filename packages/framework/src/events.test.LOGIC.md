...
- **The handoff lines, merge half included** - "✓ opened <url>" is followed by its own merge line, "✓ merged the PR"; an armed auto-merge reads "auto-merge armed: the PR lands when its checks pass"; a failed merge reads "! could not merge the PR: <error>"; a withheld merge reads "~ merge withheld: the session never signalled ready-for-merge".
