...
- **The project records the layout it uses** - the layout marker is a tracked file under `.the-framework/`, so every clone and every worktree of the project carries it. It records every name a committed artifact's path hangs off: the framework directory, the `agent-data` branch, the event log and agent meta file names, the tickets directory, the agent queue file, and the runs directory.
