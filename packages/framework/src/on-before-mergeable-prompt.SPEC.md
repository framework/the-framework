...
- **Queues quality work, never runs it** - the prompt tells the agent to put entries on the agent queue (`TODO_AGENTS.md`) with the `queue` skill's own command, one per quality preset; a later routine drains them.
