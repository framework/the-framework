...
- **One slot per project and error kind** - each project holds at most one live error per kind; today the only kind is `data-sync`, raised when a project's `agent-data` branch cannot be synced (the push to origin is rejected, or the repo has no origin at all).
