...
- **Activated means the ignore file exists** - a repository with `.the-framework/.gitignore` is activated; without that file it is not, so a `.the-framework/` directory on its own is not activation.
