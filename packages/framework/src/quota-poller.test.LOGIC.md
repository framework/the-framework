...
- **Recovering after a bad first read** - a first reading in an unrecognized shape followed by a good one leaves polling running, with the good reading retained and the gap back at five minutes, so the usage bar comes back instead of staying blank for the daemon's whole life.
- **An authoritative failure ends polling** - an account with no subscription quota stops polling and discards the retained good reading and its time; a coding agent that is not installed stops it as well.
- **A driver that fails outright** - a read that throws counts as a failed fetch, the attempt tells nothing, and polling goes on.
- **Stopping is final** - stopping twice is harmless, and starting again after a stop does not revive polling.
