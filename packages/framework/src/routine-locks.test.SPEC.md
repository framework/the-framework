What the tests cover: the routine lock — the file that reserves one of Auto PM's routines on the logs branch.
...
- **Two machines, one shared logs branch, against real git.** The first machine's lock reaches the shared branch; the second machine reads it there and stands down; the second cannot release what the first holds; and once the first machine's release reaches the branch, the second's next attempt takes the routine.
