Every read of a project's agents [1], of its agent queue [6] and of its tickets [8]. The Framework runs no agent and writes no agent's record: the tool that runs an agent keeps the agent's card and diary [2] in the agent's checkout [3] while it works, and a finished agent is whatever the project's runs provider [4] answers, when one of its packages provides one; the queue, likewise, is whatever the project's queue provider [7] answers, and the tickets whatever its tickets provider [9] answers. This directory reads all of it, owns the shape of the card, the diary, a queue entry and a ticket's row and maps the card and diary onto what the dashboard draws, and owns the rule that resolves an agent id [5] to the checkout and the diary it addresses.
...
- **The tickets provider** (`tickets.ts`) - which of the project's packages provides the tickets [8], the one command line it answers (`list --local`, every open ticket's row), the shape of a row, and five seconds of shared reads; The Framework never writes a ticket, and reads them only for what it composes across skills (the hot-tickets card, the onboarding step).
...
- **What the tests prove** (`agent-store.test.ts`, `run-record.test.ts`, `agent-checkout.test.ts`, `runs.test.ts`, `queue.test.ts`, `tickets.test.ts`) - every rule above, pinned against an in-memory file system, a throwaway project directory, a provider held in memory (`test-runs.ts`) or a real provider command.
