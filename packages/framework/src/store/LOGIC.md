Every read of a project's agents [1]. The Framework runs no agent and writes no agent's record: the tool that runs an agent keeps the agent's card and diary [2] in the agent's checkout [3] while it works, and a finished agent is whatever the project's runs provider [4] answers, when one of its packages provides one. This directory reads both places, owns the shape of the card and the diary and maps it onto what the dashboard draws, and owns the rule that resolves an agent id [5] to the checkout and the diary it addresses.
...
- **What the tests prove** (`agent-store.test.ts`, `run-record.test.ts`, `agent-checkout.test.ts`, `runs.test.ts`) - every rule above, pinned against an in-memory file system, a throwaway project directory, a provider held in memory (`test-runs.ts`) or a real provider command.
