...
- **The two shapes, mapped** (`run-record.ts`) - a card [2] becomes the dashboard's record of an agent [1], and each diary line the event the dashboard draws, carrying the time the line was written when the line has one.
