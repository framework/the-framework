How The Framework reads a project's pull requests and acts on them: through the forge provider [1], a command one of the project's own packages declares, never through a forge's own tool and never through a package The Framework knows. It also owns the shape of what that command answers: a pull request [2], and the project's page on the forge [3].

## Context

**User story**:
- The user opens a pull request for an ended agent's branch ("Open PR") or merges the one it has ("Merge PR"); the dashboard finds the pull requests a project has. A project on GitHub has the GitHub package installed; a project on another forge installs that forge's package instead, and nothing in The Framework changes.
- A project none of whose packages provides a forge has no pull requests: nothing is opened or landed for it, and an ended agent's last step is the push. The answer is "no forge", never an error.

**Business logic story**: The Framework names no forge. A project picks the package that speaks to its forge by listing it as a dependency; that package says, in its own package.json, which of its commands answers for the forge. The branches provider (`branches.ts`) pushes; this provider opens and lands. Opening a pull request from the dashboard is the two composed: the push, then the open (`../dashboard/agent-handoff.ts`).

**Problem**: "the branch has no pull request" and "the forge could not be asked" are different answers, and a caller that takes the second for the first opens a second pull request or announces every open one as new.

## Glossary

[1] forge provider: the command, among the commands of a project's dependencies, that a package declares as answering for the project's forge, in its own package.json under `"framework": { "forge": "<command>" }` (the `github` skill's package declares its `github` command); it lists the project's pull requests, opens one, lands one, and names the project's page on the forge (`store/forge.ts`).
...
[3] the project's page on the forge: a link to the project where its pull requests live, and the forge's name, for the dashboard to draw a link from without naming any forge itself.

## Business logic — TL;DR

- **Which command provides** - the shared library's rule: the one dependency that declares a forge provider [1] naming one of its own commands, or, when several declare it, the one the project's own package.json names; no dependency declares one, the project has no forge.
- **The command line it answers** - the project's pull requests (all, one branch's, by state, since a time), the opening of a branch's pull request, the landing of one, and the project's page on the forge.
...
- **The provider is looked up again every five seconds** - so a project that installs, swaps or drops its forge package is read the new way within five seconds; being told the project changed forgets it at once.
...
- `<command> home`: the project's page on the forge [3], its link and the forge's name; a refusal, or an answer without both, is nothing.
...
**Problem**: the dashboard asks for the forge on every handoff read, and looking a provider up reads the project's package files each time.
