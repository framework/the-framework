...
- **The diary** - a diary reads as events: a `said` line is the coding agent's text, a `result` line a turn's final answer keeping its session id, a `cost` line a usage event with its price, an `ended` line an end (done, stopped with its detail, failed with its detail); every other line is the event as written.
...
- **A line's time** - a `said`, `result`, `ended` and `screen` line's `at` is on the event each reads as; a line with no `at` reads with no time.
