...
- **A diary agent-driver's own log wrote** - its start, action and notice lines read as the driver's events, its session line as the session id, its question line as the gate the dashboard shows (title, options, recommendation), and an `ended` line saying waiting as an end that waits.
