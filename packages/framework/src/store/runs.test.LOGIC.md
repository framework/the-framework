...
- **Which command provides** - the first dependency, in package.json order, whose declaration names one of its own commands; a declaration naming a command the package lacks is skipped; another kind of data is provided by nobody; a project with no declaration has no provider at all.
