...
Two of the docs are not the agent's to edit by hand — `tickets/**.md` and `TODO_AGENTS.md` — because they live on the `agent-data` branch rather than in the agent's checkout. Their bullets say so and send the agent to the `tickets` skill, which is where their format, and every way of reading and changing them, is written down. No format text of their own rides in the channel. The user's picked in-context directories, when any, come first on the `Context:` line, framing whatever follows.
