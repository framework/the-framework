...
- The built-in prompt template carries its section headings verbatim, names `TODO_AGENTS.md` via the shared constant, names the session through the branch-management command rather than a branch the agent creates (the workspace rules are the skill's, not the template's), no longer asks for the retired analysis artifact or carries pre-rewrite headings, and has exactly one variable slot (the user's prompt).
...
- Composition order and exactness: context (user dirs first, docs after), format specs, built-in prompt, the branch-management skill, user prompt, then the await and signal protocols — and nothing else, whatever the options; the signal protocol is always last.
- The branch-management skill is the package's `SKILL.md` with its front matter dropped; it names the `name` and `status` commands, follows the prompt's session-name step and precedes the user's own prompt, and vanilla drops it along with the built-in prompt; a hands-off agent never gets it and is told the command is not there.
