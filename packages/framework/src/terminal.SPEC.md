...
- **One event, one line** - every event kind has a wording: session start, session name, branch, ticket being implemented, preview and browser URLs, gates (a question with its options, the recommendation or checked defaults marked), picks, driver output, usage, handoff, errors, and the end state (finished / stopped / failed).
