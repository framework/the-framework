...
Deleting an agent is the sibling of reclaiming its checkout, and the difference is the point. Reclaiming takes the checkout back and keeps the agent — its row and its replayable log — because the history was already archived. Deleting removes that archive as well: the agent meta that puts the row in the list and the event log that replays it, wherever they are filed. A run on the data branch is deleted by the `logs` skill, card and diary, inside the branch's write cycle, so the deletion is itself a committed and pushed change; a transient copy is simply unlinked, looked up rather than derived from the agent id. A file that is already gone is not an error, so a half-deleted agent still finishes cleanly.
