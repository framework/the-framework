...
- **Deleting an agent** - an agent that has only its record, its checkout already gone, is deleted cleanly through the project's runs provider (the `logs` skill's package, installed as a dependency) and the record is gone, and deleting it again still finishes cleanly; an id that is not a safe agent id is refused before anything is touched.
