...
- Whenever a checkout's branch differs from its folder name (after a rename, a continue
  onto a named branch, or the agent switching branches), a link named as the branch is put
  beside the folder, so `.branches/<branch>` reaches every checkout by its current branch;
  the package makes and removes it. `list` and `prune` see directories only, so a link is
  never a checkout.
