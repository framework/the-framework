...
- A checkout starts as branch `agent-<id>` in folder `.branches/agent-<id>/`; `<id>` is
  ...
  folder keeps the id, since the agent is running inside it.
- After a rename, a link named as the branch the checkout is on now, `agent-*` or not, is
  put beside the folder, so `.branches/agent-<name>` reaches every checkout by its current
  branch; the package makes and removes it. `list` and `prune` see directories only, so a
  link is never a checkout.
...
- The user's installed dependencies are linked into the checkout, not copied or
  ...
  Every dependency folder down to two levels under the root is linked, so a workspace
  package's own dependencies are there too. Of the dot-entries only `.bin` is linked: the
  ...
  packages resolve without it, a link to a link resolving where the target lives.
...
- The command's `remove` and `prune` push by default; `--no-push` opts out.
- The package does git and the filesystem only: whether it may push comes in from the
  caller; the package never reads that program's files.
