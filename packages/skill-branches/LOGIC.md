The `branches` skill [1], one of the four skills every agent [2] is taught: one git worktree per agent under the project's `.branches/` directory, the agent's checkout [3], named as its branch and reclaimed [4] once its work is on the remote, and pushed [7] by the agent itself when it finishes. The package knows git and nothing beyond it: what becomes of a pushed branch on the project's git host, a pull request say, is another skill's command, and this package names no git host. The package is three things at once: the instructions the agent reads (`SKILL.md`), the `branches` command every agent runs through `npx` (`bin/`, `src/cli.ts`), and the library the runner (`agent-runner`) calls to make and reclaim checkouts (`src/`). The dashboard's server imports nothing of it: `package.json` declares the `branches` command as the framework's branches provider [8], and the dashboard's server runs that command to list checkouts, read what a finished agent's branch holds, push it for a person, and remove or discard a checkout; opening the pushed branch's pull request is the git host provider's command, which the dashboard's server runs after this one. `package.json` and the `tsconfig*.json` files otherwise configure the package, its build and its tests, `dist/` and `dist-test/` are build output, and `DECISIONS.md` records the decisions the code implements; none of them carries business logic of its own.
...
When the runner makes an agent's [2] checkout [3] through the library, the library links this package's directory, the one holding `SKILL.md`, into the checkout where each coding agent [6] looks for skills [1] (`src/`). The agent reads `SKILL.md`, learns that it runs `npx branches` inside its checkout, and uses `status`, `name` and `push`; `create`, `attach`, `show`, `list`, `remove`, `prune` and `push --branch` belong to whoever started it: the runner calls the same operations directly as a library, and the dashboard's server runs the command as the branches provider [8]. One implementation serves every surface: the agent in its shell, the runner at a run's end and in its sweep, and the dashboard's "Push", "Open PR" (its push half) and "Remove" buttons.
