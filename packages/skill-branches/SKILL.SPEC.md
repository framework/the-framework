...
- **On its own branch, the working directory is the whole workspace** - every file the agent reads or writes is under it; dependency files and the skill folders are the user's copies, linked in, and are never edited; anything it genuinely needs from outside is a reason to say so and stop. Before the first change it names the session with `npx branches name <name>` — a rename of its branch to `agent-<name>`, suffixed when the name was taken, printed as `branch`, another name when refused — unless the branch is already named, which a continued agent's is: then it keeps it.
