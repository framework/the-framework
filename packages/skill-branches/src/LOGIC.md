Implements the `branches` skill [1]: one git worktree per agent [2] under the project's `.branches/` directory, the agent's checkout [3], named as its branch; made with everything an agent needs, renamed after the work once the agent names it, listed and sized for the dashboard, pushed by the agent itself when it finishes, and reclaimed [4] once its work is on the remote, so nothing local is ever the last copy of anything. Git only: what becomes of a pushed branch on the project's git host is another package's command, never named here. The runner (`agent-runner`) calls it as a library, every agent runs it as the `branches` command, and the dashboard's server runs the same command as the project's branches provider [15]: to list checkouts, read what a finished agent's branch holds, push it for a person, and remove or discard a checkout. The `*.BUG-ANALYSIS.md` notes beside the sources are review bookkeeping and carry no business logic.
...
- **Reclaiming** (`reclaim.ts`, `reclaim.test.ts`) - only what is on the remote may go: the refusals `not-a-worktree`, `no-branch`, `dirty` and `not-on-remote`, the push when the caller allows it, and the branches that go with a reclaimed [4] checkout; and a person's discard, the checkout gone whatever it holds, the branch kept.
- **A branch's state** (`branch-state.ts`, `branch-state.test.ts`) - what a branch holds beyond the base and where it stands: its commits and files, pushed, merged, the checkout's uncommitted paths; git facts only, the pull request left to the caller.
- **Pushing** (`push.ts`, `push.test.ts`) - the agent's own last git step: a clean checkout's branch pushed to `origin`; a dirty checkout refused before anything is pushed. A person pushes a finished run's branch by name through the same door: the checkout on it under the same rule, else the branch itself, left as it is when only `origin` has it.
- **The `branches` command** (`cli.ts`, `cli.test.ts`, `push.test.ts`) - `create`, `attach`, `name`, `status`, `show`, `push`, `list`, `remove` and `prune`: one JSON document on stdout, one line on stderr, exit code 0, 1 or 2; the dashboard's server runs it as the branches provider [15].
...
- **The entry point** (`index.ts`) - what the runner and the dashboard's server import, and the naming rules alone for the dashboard's browser code.
...
- The agent pushes its branch when its work is committed (`branches push`, `push.ts`); the dashboard's server, running the command as the branches provider [15], lists the checkouts on disk with their branch and size (`branches list`), reads what a finished agent's branch holds (`branches show`, `branch-state.ts`), and, when the user asks, pushes the branch (`branches push --branch`, `push.ts`); whatever the project's git host does with the pushed branch is the git host provider's command, run after this one.
- Once the agent is done, the runner's run at its end or its sweep [12], or `branches remove` (the dashboard's "Remove" button runs it) reclaims [4] the checkout under the one rule (`reclaim.ts`): a clean tree whose tip is on the remote, pushed on the way when allowed; a branch that held nothing and the birth branch the agent left behind go with it. A run the user deletes is discarded with `branches remove --discard`, whatever its checkout holds. `branches remove` reconciles the branch links again; after the runner a stale link goes at the next reconcile.
