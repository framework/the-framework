Implements the `branches` skill [1]: one git worktree per agent [2] under the project's `.branches/` directory, the agent's checkout [3], named as its branch; made with everything an agent needs, renamed after the work once the agent names it, listed and sized for the dashboard, published by the agent itself when it finishes, and reclaimed [4] once its work is on the remote, so nothing local is ever the last copy of anything. The scheduler (`agent-scheduler`) calls it as a library, every agent runs it as the `branches` command, and the dashboard's server runs the same command as the project's branches provider [15]: to list checkouts, read what a finished agent's branch holds, publish it and land its request for a person, and remove or discard a checkout. The `*.BUG-ANALYSIS.md` notes beside the sources are review bookkeeping and carry no business logic.
...
- **The `branches` command** (`cli.ts`, `cli.test.ts`) - `create`, `attach`, `name`, `status`, `show`, `publish`, `merge`, `merge-on-green`, `release`, `list`, `remove` and `prune`: one JSON document on stdout, one line on stderr, exit code 0, 1 or 2; the dashboard's server runs it as the branches provider [15].
...
- The dashboard's server, running the command as the branches provider [15], lists the checkouts on disk with their branch and size (`branches list`), reads what a finished agent's branch holds (`branches show`, `branch-state.ts`), and, when the user asks, publishes the branch (`branches publish --branch`) or lands its request (`branches merge`) (`publish.ts`).
