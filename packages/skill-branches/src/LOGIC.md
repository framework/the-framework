Implements the `branches` skill [1]: one git worktree per agent [2] under the project's `.branches/` directory, the agent's checkout [3], named as its branch; made with everything an agent needs, renamed after the work once the agent names it, listed and sized for the dashboard, pushed by the agent itself when it finishes, and reclaimed [4] once its work is on the remote, so nothing local is ever the last copy of anything. Git only: what becomes of a pushed branch on the project's git host is another package's command, never named here. The scheduler (`agent-scheduler`) calls it as a library, every agent runs it as the `branches` command, and the dashboard's server runs the same command as the project's branches provider [15]: to list checkouts, read what a finished agent's branch holds, push it for a person, and remove or discard a checkout. The `*.BUG-ANALYSIS.md` notes beside the sources are review bookkeeping and carry no business logic.
