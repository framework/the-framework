...
**User story**: an agent publishes its work with `--merge` in a repository whose settings do not allow auto-merge; the request lands once its checks pass, the same as where auto-merge is on, and a red request stays open for a person.

**Problem**: the agent's session ends right after it publishes, and nothing else runs for the request. The watcher keeps nothing but the request's number: the request on GitHub is the whole state, so a watcher that dies (a reboot) leaves an open request a person can merge, never a wrong merge.
...
- **Its own process** - started detached from the project's root as `branches merge-on-green <number>`, its output appended to `.branches/merge-on-green/<number>.log`; it outlives the agent that published.
