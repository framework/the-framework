...
- **When it ends** - on `close`; on an `ended` line reaching the diary after the process started, unless it says the agent ended waiting on a question; on the diary's file going away; after 30 minutes (the caller's figure) with no command and no input; when Chrome exits, or the process is told to stop.
...
- When it was given a diary: the diary's file gone (the agent's checkout reclaimed) counts as the agent having ended, and it ends. Otherwise the whole lines written since the last check (since the process started, for the first), up to the last line break, are read, counted in bytes; a line still being written is left for the next check, when it is read complete. Each line is read as JSON, a line that is not JSON skipped; a line whose `kind` is `ended`, the line the tool running the agent appends when the agent ends, counts as the agent having ended, and it ends, unless its `status` is `waiting`: an agent that stopped on a question keeps its browser for the answer (the 30-minute limit still holds). A diary written back shorter than where the check had got to (an agent resumed from its record) is followed from its new end. An `ended` line written before the process started, such as an earlier session's, does not count.
