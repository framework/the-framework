...
- **`requests`** - the flags become one gh listing (state `all` by default, `--branch` as the head, `--state` and `--since` applied), the rows come back as requests newest first, and a gh that cannot answer is the refusal `git-host-failed` with gh's line, exit 1.
...
- **`home`** - the project page and the git host's name from a GitHub origin; another host or no origin is the refusal `no-remote`.
