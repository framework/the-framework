...
- **One read** - the request's state and its checks [1]: `passing` when every check concluded and none failed (skipped and neutral count as passing), `failing` when any concluded check has a verdict other than success, neutral or skipped (failed, cancelled, timed out, action required, stale, error), even while others still run; `pending` while any still runs and none failed; `none` when no check is reported; a read gh cannot answer counts as `pending`, so an unreadable status never merges; a read with no state reads as open.
...
- **Its own process** - started detached as `github watch <number>` in the directory the command ran in, its output appended to `.branches/merge-on-green/<number>.log` under that directory; it outlives the agent that opened the request.
