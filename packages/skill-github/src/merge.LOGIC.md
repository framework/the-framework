...
- **Landing by number** - a draft is marked ready first, since asking for the merge is the statement that its review happened; then the merge is armed exactly as `open --merge` arms it; a request that is not open is `not-open` with its state, since already merged is an answer, not an action; a request gh cannot read is `failed`.
