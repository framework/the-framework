...
- **The request** - `gh pr create` with the head branch, the title and the body given (an empty body when none); as a draft only when asked and the merge is not armed, since a draft cannot merge; the number is read off the URL gh prints, else off a second listing, and when neither finds it the answer is `open-failed` ("gh printed no pull request URL") although gh may have opened it; gh refusing (a branch not on the remote, no commits against the base) is `open-failed` with gh's line.
