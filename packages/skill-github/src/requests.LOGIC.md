...
- **Since** - `--since <time>` keeps the requests created at or after it; when only merged ones were asked for, the ones merged at or after it. The times are compared as text, so `--since` must be in gh's own UTC form (`2026-09-25T10:00:00Z`).
