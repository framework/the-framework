...
- **The shape** - states lowercased, the head as `branch` and `head`, `draft`, `mergedAt` and `mergeCommit` only when set; a row without a number and a url is dropped; a non-list is no request.
