...
- **A run recorded again stays where it sits** - a run another machine filed under its own person, left marked `running`, is ended by this machine in place, under that person's directory, with its diary rewritten to the lines this machine gives; a card that does not parse is skipped from the listing.
