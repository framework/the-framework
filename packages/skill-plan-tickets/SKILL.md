...
4. Take at most ten: highest priority first, then oldest date first. The rest wait for the next run.
5. Queue them one by one in that order, each as the sentence `Create tickets/<name>.plan.md` at the ticket's priority. If a write fails, show an error to the user saying which ticket and why, and stop: that ticket and the rest wait for the next run.
