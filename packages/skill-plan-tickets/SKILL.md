...
4. A ticket's priority is its own when that is a bare whole number from 0 to 10, else 5. Take at most ten: highest priority first, then oldest date first, then by the listing's file name, `.md` included, A to Z. The rest wait for the next run.
5. Queue them one by one in that order, each as the plain sentence `Create tickets/<name>.plan.md` at the ticket's priority. Not a link: a link would ask for the ticket's work, not its plan. If a write fails, show an error to the user saying which ticket and why, and stop: the rest wait for the next run.
6. End by naming each entry you queued with its priority, each ticket you skipped and why, and how many wait for the next run.
