...
/**
 * Where the `queue` executable lives, for a caller that puts it on a spawned process's PATH — a
 * daemon, for every agent it starts. Beside `dist/`, so it is the same path from a workspace
