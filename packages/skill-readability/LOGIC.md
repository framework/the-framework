The `@gemstack/skill-readability` npm package: the `readability` command skill [1], one `SKILL.md` and no code. A command skill is a job for a coding agent [2] that composes the capability skills [3] of a project; it names no skill, no command and no package, and it is marked so that only a person or a runner invokes it, never the agent by itself. One package per command, named `@gemstack/skill-<command>`. A project tracks the skill file as `.claude/skills/readability/SKILL.md`; a person fires it from the dashboard's launcher, picking it from the `/` list or typing its slash command, `/readability`. Nothing depends on the package.
