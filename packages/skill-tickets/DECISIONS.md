...
- Tickets live on the data branch, never on a code branch: an agent's checkout has no
  `tickets/` folder, and a pull request never carries a ticket. For a person, a
  `tickets` shortcut at the project root points into the data branch's checkout; it is
  created only if nothing of that name is there, and git is told to ignore it.
