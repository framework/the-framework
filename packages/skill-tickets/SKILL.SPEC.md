...
- **The `tickets` command is the only way to change them** - every change it makes is one commit pushed straight to the `tickets` branch; nothing the agent commits on its own branch reaches them, and these files never belong on the agent's branch.
