...
- **The tickets are on a branch, not in your checkout** - the tickets (`tickets/<DATE>_<SLUG>.md`, with their `.plan.md` and `.lock.md` siblings) and the queue (`TODO_AGENTS.md`) live on the `tickets` branch, never on a code branch; the agent's own checkout does not contain them. A `tickets` link at the repository root may show them: read there if you like, never write there.
- **The `tickets` command is the only way to change them** - it ships with the `@gemstack/skill-tickets` package the repository depends on: the agent installs the repository's dependencies once and runs `npx tickets`, so every command the skill names runs as written on a fresh clone. Every change it makes is one commit pushed straight to the `tickets` branch; nothing the agent commits on its own branch reaches them, and these files never belong on the agent's branch.
