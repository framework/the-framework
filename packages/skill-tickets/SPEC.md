Tickets and the agent queue for coding agents, as an npm package: markdown tickets with their plans and their claims, and the priority-banded list of what agents work on next — all of it on the `tickets` branch of the project's own repository, never on a code branch; the `tickets` command that reads and changes them from any clone; and the instructions an agent follows to use them (`SKILL.md`).
...
- **The conventions** (`names`) - the branch `tickets`, its persistent checkout `.branches/tickets`, the `tickets/` folder, the queue file `TODO_AGENTS.md`, the `meta.json` stamp; how a ticket's filename names its plan and its claim; the gates every filename arriving from outside passes; the ticket a queue entry links back to; the queue section a ticket's own priority earns it; the issue a ticket tracks.
...
- **Who claims** (`holder`) - the identity a claim is made under, read from where the command runs: the agent id inside an agent's checkout, else the current branch name; a detached checkout has none.
