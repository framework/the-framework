...
- **The plan, addressed by its ticket** - the ticket's own file name decides which plan is read, with the ticket, and the page names the plan's path.
...
Until the first read returns, the page reads "Loading…". When no plan is there — never written, or removed since the list was read, or the ticket itself is gone — the page reads "This ticket has no plan yet.". When the command could not run at all, the page names its reason: "Could not read the plan: <reason>". Starting an agent [1] to write the plan is offered on the ticket's row in the list, not here.
...
The agent that wrote a plan is found among the project's runs the dashboard knows (the ones running, and the recorded ones when the project records them): the newest whose ask named this exact plan — the "Create tickets/<stem>.plan.md" wording every surface uses to ask for a plan. An agent whose ask never named the plan, such as one that worked the agent queue's first open entry without naming it, is not attributed, and the page then shows no such offer. The attribution is re-read every 10 seconds.
