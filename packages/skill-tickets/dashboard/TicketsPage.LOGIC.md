...
- **Add the shown tickets** - one button per link action the installed widgets offer ("Add to queue: all N tickets shown below") hands every shown, or selected, ticket that is neither claimed nor in review nor waiting as a link to the action (what is already queued is the action's own to skip), and then reads "Queued".
- **Add plans for the unplanned ones** - a second button per action ("Add to queue: plans for all N tickets shown below") hands the plan ask for every shown, or selected, ticket that is neither planned nor claimed nor in review nor waiting, and then reads "Queued"; without any widget offering an action on links, neither button exists.
...
The button's set is the shown rows, in the order shown, narrowed to the ticked rows while any is ticked, minus every claimed [4] ticket: a ticket an agent [1] holds is being worked, and its entry would outlive that work as noise on the agent queue [2]. Every ticket in review or waiting is left out too, by the rule in `src/widget.ts`: no agent is handed one.

The buttons are the dashboard's slot for the installed widgets' [9] link actions, given the projects of the set: one button per action whose package at least one of those projects has, none when no widget offers an action on links. A button appears only once the tickets are loaded and the set is not empty; an empty set, or one that is all claimed, in review or waiting, is not an offer. Its text is the action's own label, then the set: it counts exactly what a click adds and stops saying "all" as soon as a ticket is skipped, claimed, in review or waiting. With the queue package's "Add to queue":
- Without a selection: "Add to queue: all N tickets shown below"; "Add to queue: the N ready tickets shown below" when tickets were skipped; for a single ticket "Add to queue: the ticket shown below" or "Add to queue: the one ready ticket shown below".
- With a selection: "Add to queue: the N selected tickets"; "Add to queue: the N ready selected tickets" when tickets were skipped; for a single ticket "Add to queue: the selected ticket" or "Add to queue: the one ready selected ticket".

Its tooltip reads "Every ticket joins the queue — the work the framework picks up on its own, worked highest priority first and, within a priority, in the order shown below. A ticket already queued stays as it is." ("Every selected ticket …" with a selection, followed by "The rest of the shown set stays put."). When one claimed ticket was skipped it adds "The claimed ticket shown is left to the agent holding it." (or "… selected …"); when several were, "The N claimed tickets shown are left to the agents holding them." Tickets in review or waiting add "The ticket in review or waiting shown is left out." or "The N tickets in review or waiting shown are left out." ("… selected …" with a selection).
...
Its tooltip reads "Each ticket gets its plan asked for — the same "Create tickets/….plan.md" entry the plan-tickets command queues — worked highest priority first and, within a priority, in the order shown below. Tickets already planned, held by an agent, in review or waiting stay as they are, and what is already there is left alone." ("Each selected ticket …" and "The rest of the shown set stays put." with a selection).
