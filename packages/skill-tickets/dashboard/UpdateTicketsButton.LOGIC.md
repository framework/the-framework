...
When the caller knows when the tickets last caught up with the issue tracker, the tooltip reads "Bring tickets/ up to date with the issues and comments changed since the last import." With no import on record it reads "Bring tickets/ up to date with the issue tracker. With no import on record, everything open comes across."
