...
**Business logic story**: the whole set of tickets across every project is already in the browser after one read of each project's `tickets` command, so filtering, counting and sorting happen on what is on screen and the command is never asked to filter. The tickets themselves come off each project's `agent-data` branch [2].
