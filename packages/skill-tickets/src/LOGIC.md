...
- **Claiming and releasing** (`locks.ts`, `locks.test.ts`) - the one-line claim file that never expires, claiming to plan or to implement, batches re-judged rather than double-claimed, releases only by the holder, who claimed a ticket before, and a commit that counts even when it could not be pushed.
