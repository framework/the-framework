...
- **Who the holder is** - the agent id inside a `.branches/agent-<id>` checkout, and that id survives the session renaming its branch; a detached checkout is refused as having no identity.
- **`put`** - a ticket, a plan and `meta.json` are written from standard input, each as one commit named after the file; a `.lock.md`, a path that climbs out, a nested path and a non-markdown name are all refused as an invalid path; the ticket just written lists, planned, with its plan's effort.
