...
- **Reads** - `list` gives every open ticket with its priority, whether it is planned and its plan's effort; `show` gives one ticket's whole text, its plan and its holder, and takes the ticket by its bare filename or by its path; all of it read off origin, from a clone holding no checkout of the branch.
