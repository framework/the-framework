...
- **The first sync** - the branch is born an orphan with nothing of the package's written on it, its persistent checkout is at `.branches/agent-data` on that branch and clean, and the paths reported are the checkout and its `tickets/` folder.
...
- **Idempotence** - a second sync links nothing new and writes nothing.
