...
- **The first sync** - the branch is born, its persistent checkout is at `.branches/agent-data` on that branch, the queue file is seeded and committed so the checkout is clean between cycles, and the paths reported are the checkout and its `tickets/` folder.
