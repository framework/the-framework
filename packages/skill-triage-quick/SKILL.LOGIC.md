...
- **Queueing** - each picked ticket goes on the agent queue as a link to the ticket labeled with its title, with a sensible priority, the lowest-effort tickets bumped so agents work them first; a ticket already on the queue or held by someone is skipped.
