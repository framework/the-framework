...
4. A ticket's priority is its own when that is a bare whole number from 0 to 10, else 5. Order the tickets lowest effort first, then by `<file>`, compared character by character in code order; with both modes, the two kinds as one list.
