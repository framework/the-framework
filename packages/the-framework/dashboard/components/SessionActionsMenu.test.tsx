...
describe('SessionActionsMenu (#toolbar-menu)', () => {
  test('folds the session actions into one menu', async () => {
    render(<SessionActionsMenu projectId="p1" agentId="run-1" events={[]} label="my session" onDeleted={vi.fn()} />)
    openMenu()
  ...
  test('opening the folder addresses this session, whichever checkout it resolves to', async () => {
    render(<SessionActionsMenu projectId="p1" agentId="run-1" events={[]} onDeleted={vi.fn()} />)
    openMenu()
  ...
  test("names the folder item for what it opens once the session's worktree is gone (#1195)", async () => {
    ...
    render(<SessionActionsMenu projectId="p1" agentId="run-1" events={[]} retainedWorktree onDeleted={vi.fn()} />)
    openMenu()
  ...
  test('copies the command that reopens the session in a terminal (#1195)', async () => {
    ...
    render(<SessionActionsMenu projectId="p1" agentId="run-1" events={events} onDeleted={vi.fn()} />)
    openMenu()
  ...
  test('Delete asks to confirm before deleting', async () => {
    ...
    render(<SessionActionsMenu projectId="p1" agentId="run-1" events={[]} label="my session" onDeleted={onDeleted} />)
    openMenu()
