...
// device's token is a per-browser secret, so which device an agent goes to is transient UI state, never
...
/** Select a saved device (by profile id) as the agent target, or null to go back to a driver target. */
...
/** The selected device id, read at submit time to attach the agent's relay target. */
