...
const { useRuns } = await import('./use-agents.js')
...
describe('useRuns', () => {
  ...
  test('a reload in flight during a project switch cannot write the old project s runs', async () => {
    ...
    onRuns.mockReturnValue(new Promise<AgentMeta[]>(r => (resolveA = r)))
    result.current.reload() // in flight against project a
    onRuns.mockResolvedValue(agents('b1'))
    rerender({ id: 'b' })
    await settle()
  ...
  test('switching project clears the previous project s runs rather than showing them', async () => {
    ...
    onRuns.mockReturnValue(new Promise<AgentMeta[]>(r => (resolveB = r)))
    rerender({ id: 'b' })
