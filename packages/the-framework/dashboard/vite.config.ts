...
export default defineConfig({
  ...
  build: {
    // Straight into the package's own dist, where the daemon serves it from (Vike writes the
    // prerendered SPA to `<outDir>/client`). There used to be a copy step between the two —
    // a whole turbo task — because the bundle was built in a different package.
