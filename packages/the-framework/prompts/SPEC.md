...
- The presets are the one-click task prompts behind the dashboard's buttons: research, market research, the quality reviews (readability, maintainability, security, UX), ticket triage and planning, updating tickets from GitHub, the three suggestion passes (new features, new tickets, which tickets to work on), the maintenance sweep, and draining the queue. A triage only writes the queue file — it never implements the tickets it picks; implementing is the drain's job.
