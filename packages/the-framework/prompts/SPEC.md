...
- The presets are the one-click task prompts behind the dashboard's buttons: research, market research, the quality reviews (readability, maintainability, security, UX), ticket triage and planning, updating tickets from GitHub, the three suggestion passes (new features, new tickets, which tickets to work on), the maintenance sweep, and draining the queue. A triage never implements the tickets it picks; implementing is the drain's job.
...
- The format docs teach the repo conventions: tickets as dated proposal files with plan and claim siblings (`.plan.md`, `.lock.md`), and the priority-ordered queue file (`TODO_AGENTS.md`) of confirmed work. One more convention is the triage scope — a triage only writes the queue file — kept as a single text that both triage presets end with.
...
- A triage is told to queue work and never do it because the queue is the one place a human can veto a ticket before an agent builds it, and because a triage rewrites the shared queue document and so runs as a single agent while the drain runs one agent per entry — a ticket implemented inside a triage skips the veto and takes the single-agent lane.
