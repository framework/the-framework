Update `tickets/` from this repo's GitHub issues, bringing across what has changed since the last import rather than starting over. `tickets/` lives on the data branch: read and write it per "The data branch", the whole update as one data-branch commit — no PR.
