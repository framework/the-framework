Pins how an agent reports on itself without stopping: the four blocks it emits mid-turn to name its session, declare the work ready for merge, dictate the pull request The Framework opens for it, and report something only the user can fix.

## User story

- The user watches a list of running agents. Each one has to say what it is working on, and when it is done, or the list is unreadable.
...
- **Session name** - emitted alongside picking the name and branching, so the dashboard shows which agent this is; re-emitted on a rename.
...
See `## User story`: the dashboard lists agents by what they are doing.

#### Business logic

When the agent calls `setSessionName()` — which it does after creating and checking out its `tf-<session name>` branch — it also emits a `set-session-name` block naming it, so the dashboard shows which session this is. The block's first non-empty line is the name, an `[a-z0-9-]` slug. The agent does not stop, and re-emits the block if it renames the session.
