...
**The idle sweep.** On a timer, per project, the daemon asks one policy question — "is now a good time to spend quota on our own roadmap?" — checking the cheapest facts first: feature on? concurrency free? cooldown elapsed? queue readable? quota headroom left? Then: a non-empty queue is drained one entry per session; an empty queue is refilled by rotating through quick triage → consensual triage → spike-and-plan. Spike-and-plan fans out several agents, each pinned to exactly one ticket and claimed via a lock file that lives beside the ticket on the default branch — so agents on other machines and cloud sessions see the claim too. A calendar-paced maintenance sweep (review the commits each repo grew since its last review) sits outside the rotation and takes precedence when due. Every refusal is phrased as a reason, so a setting never reads as a bug.
...
- **Tickets** — `tickets/<DATE>_<SLUG>.md`, the human-facing roadmap, with optional spike (`.spike.md`), plan (`.plan.md`), and claim (`.lock.md`) siblings. A plain repo convention, parsed tolerantly so imported tickets that predate the format still list.
- **The queue and the project log** — the queue file as above, and a human-readable log of what The Framework did to the project.

### Prompts and presets

One assembly path composes the system prompt for every session — the built-in protocol (analyze the prompt, name the session, measure variability, offer alternatives, signal ready-for-merge), the extra protocol each of the session's capabilities brings (gates, browser, hands-off), the user's own system file, and the picked context — and the exact composed text is recorded, so the dashboard can show precisely what the agent ran under. Presets (triage, research, security audit, drain-the-queue, …) are one catalog with prompt text authored as prose; custom presets save to either the user tier (follows the person, private) or the project tier (travels with the repo, shared). A per-repo config file records which domain preset and modes a project builds under, resolved layer over layer so a project overrides only what it actually sets.
