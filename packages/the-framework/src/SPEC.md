...
- **Autonomy, bounded** - on one shared background clock the daemon runs Auto PM (drain the agent queue, triage, plan), the CI watch (merge on green, fix on red), and the sweeps (reclaim pushed checkouts, release stale branches, adopt cloud work, expire dead refs) — each start gated by the quota boundary.
