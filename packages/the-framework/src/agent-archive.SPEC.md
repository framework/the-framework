...
- That history used to live only in untracked state, so an ordinary `git clean` erased every agent a project had ever run; keeping the archive in tracked files is the fix. For the same reason the directory's earlier name is still read: a rename that hid the archives written before it would lose exactly what this exists to keep.
