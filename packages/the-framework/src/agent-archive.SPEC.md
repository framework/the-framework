...
- That history used to live only in untracked state, so an ordinary `git clean` erased every agent a project had ever run; keeping the archive in tracked files is the fix.
