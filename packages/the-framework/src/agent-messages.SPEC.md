...
- A daemon-managed session ends itself when the queue is idle — a later message reopens the conversation — while an agent whose own terminal is the only surface stays parked, since it has no daemon to resume through.
- Stopping or closing wakes every waiter empty so the agent ends cleanly, and a stale message never starts a turn on an aborted run.
