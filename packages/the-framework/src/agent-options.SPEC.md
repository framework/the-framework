Turns the user's saved preferences into the options an agent starts with, so an unattended run honours exactly the same settings as one started from the dashboard.
