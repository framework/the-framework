...
- JSON has a real `false`, so an option that defaults on can simply be off — a command line can only say present-or-absent, which forces paired on/off flag spellings and a three-way resolution behind them.
