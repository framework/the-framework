...
- The ticketless hand-off window is accepted rather than closed: the durable per-entry claim it would take was deleted as a read-time guess, tickets — the queue's normal case — are covered by the pushed lock, and the queue's planned move onto an eagerly-pushed data branch closes the window structurally. A second claim shape built now would be deleted then.
