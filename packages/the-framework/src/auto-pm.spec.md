...
- `pinnedSpikeJob()` — a fanned-out [Spike & plan] pinned to one locked ticket (#1327): the pin and the agent's own `PENDING:<AGENT_ID>` claim are *appended* to the stock prompt, so the preset's verdict rules ride along verbatim and a rewritten preset cannot silently lose the pin.
