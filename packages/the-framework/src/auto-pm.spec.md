...
- `pinnedSpikeJob()` — a fanned-out [Spike & plan] pinned to one locked ticket (#1327): the pin and the agent's own `.lock.md` claim (`CLAIMED: <AGENT_ID>`, #1420) are *appended* to the stock prompt, so the preset's verdict rules ride along verbatim and a rewritten preset cannot silently lose the pin. The agent is told to delete the lock in the same commit as its plan — nothing else releases it.
- `startAutoPm(deps)` — the tick loop: promote finished runs' queues (#852), decide per project, pick the job, fan drains — and the `fansOut` spike job (#1327) — out to concurrency, log and record an `AutoPmReport` (#1161). Every reading/effect is injected via `AutoPmDeps` so the loop tests off disk.
...
- Fan-out is a property of what a job does to shared files (#1204/#1327): a drain takes one entry *off* the queue, a pinned spike writes one ticket's *own* siblings — disjoint work, disjoint edits. Every other rotation job rewrites the whole queue file from the same fork point, so two at once would have the later promotion revert the earlier; those stay one run per tick however high the concurrency.
- The spike fan-out requires both seams (`spikeCandidates` + `lockSpikes`) and live locks (#1327): candidates without locks would fan out unguarded, so anything missing or failing falls back to the stock single unpinned agent — the pre-#1327 behaviour, safe without a lock. Agent ids are clock+index (`spike-<now>-<i>`), generated in the sweep because the id must reach both the lock file and the pinned prompt. No candidates left advances the rotation (the job's work is done, not refused); a fanned run's ticket joins the in-memory pins so a later tick does not re-offer it while its lock commit may not have landed remotely.
