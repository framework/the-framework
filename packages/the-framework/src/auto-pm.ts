...
export interface SpikeAssignment {
  ...
  ticket: string
  /** What the lock files' `PENDING:<AGENT_ID>` line carries, so an agent can tell its own claim
   *  from another's. Generated by the sweep, since no run id exists before the run starts. */
  agentId: string
}
 ...
 *
 * The agent is also told which claim is its own: its ticket's siblings already exist as
 * `PENDING:<agentId>` placeholders the daemon pushed, and finding anything else there means the
 * assignment is stale — another agent's claim, or a human's real spike — so it stops.
 */
export function pinnedSpikeJob(job: AutoPmJob, assignment: SpikeAssignment): AutoPmJob {
  ...
  return {
    ...
    prompt: [
      ...
      '',
      `Your claim on it is already in place: \`tickets/${stem}.spike.md\` and \`tickets/${stem}.plan.md\` exist as placeholders reading \`PENDING:${agentId}\`. Replace both placeholders with the real spike and plan. If either file instead holds real content, or a PENDING line naming a different agent, the ticket is not yours — stop and do nothing.`,
    ].join('\n'),
...
export interface AutoPmDeps {
  ...
  /**
   * The tickets open for a spike (#1327): no spike or plan yet — real or PENDING — most important
   * first, as filenames inside `tickets/`. Asked only when the tick lands on a
   * {@link AutoPmJob.fansOut} job. The daemon-side implementation releases stale locks before it
   * enumerates, so a crashed agent's ticket comes back on the market by itself. Unreadable means
   * none, and no seam at all means the stock single agent: the fan-out is an addition, not a
   * precondition, and a loop wired without it behaves exactly as before #1327.
   */
  spikeCandidates?(project: AutoPmProject): Promise<readonly string[]>
  /**
   * Claim `assignments`' tickets before their agents start (#1327): placeholder `.spike.md`/
   * `.plan.md` siblings reading `PENDING:<AGENT_ID>`, committed as one batch and pushed to the
   * default branch, so agents forked from any checkout — and cloud sessions, which is the point —
   * find the files and skip the ticket. Resolves the subset actually locked: a ticket lost to a
   * race locks fewer, and each missing lock costs one agent of the batch rather than the batch.
    ...
    try {
      ...
      for (const project of projects) {
        ...
        if (decision.mode === 'drain') {
          ...
          // two places that must agree: the lock file's PENDING line and the pinned prompt that
