...
- A failed merge is remembered in-memory (`attemptedMerges`) and not retried until a daemon restart: branch protection demanding a review must cost one `gh` write, not one per tick for a week. Its log line is said once (`sayOnce`), as is the attempts-exhausted line.
