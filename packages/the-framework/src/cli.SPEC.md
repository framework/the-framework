...
- A live session gets everything around the agent: settings resolved across config layers, prerequisite checks, persisted events, the steering channel, the browser, the spend gate, live chat, and the committed conversation. It serves no dashboard of its own — the one that spawned it reads its event log off disk and steers it through the control file.
