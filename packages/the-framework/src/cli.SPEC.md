...
- One more, not for humans: `--session <path>` runs the agent described by a JSON spec. That is how the dashboard spawns one, and it replaced sixty-seven flags — twenty-seven of which had no human user at all, being `StartRunOptions` serialized onto a command line.
