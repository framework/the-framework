...
- A live session gets everything around the agent: settings resolved across config layers, persisted events, the steering channel, the browser, the spend gate, and live chat. It serves no dashboard of its own — the one that spawned it reads its event log off disk and steers it through the control file.
