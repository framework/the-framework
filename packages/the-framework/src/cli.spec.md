...
- Handoff defaults ON (#1102): a session left alone pushes its branch and opens a draft PR; the armed pair is mutable via control-channel `handoff` entries and mirrored to meta as `handoff-armed` events (only events reach a tab opened mid-run). A stopped run never hands off; `commitPendingWork` runs first only in a framework-owned checkout (`--run-id`).
