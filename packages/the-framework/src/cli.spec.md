...
- Steerable vs interactive (#905): "is a daemon alive" is a machine-global file wrong in both directions — missing while the daemon lives (#922: Stop presses written to control.jsonl, tailed by nobody) and present for unrelated runs (a terminal `--no-dashboard` run parked forever in the #714 chat loop). Split: steerable = own dashboard ∨ `--run-id` ∨ daemon alive; interactive (gets the chat queue) = own dashboard ∨ `--run-id` only. Of those, only a run with no `--run-id` (its own terminal dashboard is the single surface) keeps the stay-open park — a daemon-spawned run drains what queued and ends itself (#1390), reopened via `--resume`.
...
- `resumeSession` is rejected on a build run (exit 2) rather than silently dropped: a fresh session that looks resumed is the worst outcome (#782). Exception (#1467): with `--continue-run` the reopened meta's recorded `kind` decides the path — a continuation of a `build` run re-enters the build flow via `runFramework({resumeSessionId})` (journal + log entry say `build`), while a `prompt` run's continuation (and any run recorded before `RunMeta.kind` existed) keeps the direct prompt path. The composer's Resume always arrives as a `prompt` start; the CLI self-corrects from the run's own record, so neither the daemon nor the dashboard needed to change.
