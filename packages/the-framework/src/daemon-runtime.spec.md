...
- Before any of that, `onStart` preflights the picked agent (#1326): installed, logged in, not root. A failure refuses the Start with the fix, so a doomed run spends no branch and no worktree (#1323). `actions` runs are exempt (they execute on a GitHub Actions runner and need no local CLI); a *pass* is cached for 30s so a burst of starts pays for the probes once, while a *failure* is re-probed every time so logging in lands on the very next Start. `agentPreflight` is the test seam, also forwarded by `runDaemon`, so a suite exercises worktree logic rather than whether the machine running it has an agent installed.
...
- A continuation start overlays the project's resolved run options under the caller's explicit ones (#1467): the composer's Resume sends only `{resumeSession, continueRunId, agent}`, so without the overlay the run's armed handoff fell back to bare defaults — a first leg armed merge-on-green resumed with the merge silently disarmed and ended in a draft PR. Same layering the daemon-restart resume (`resumeSuspendedRuns`) has always applied; fresh starts are untouched (the launcher sends its options whole).
...
- Tri-state flags (#841/#842): explicit `false` emits `--no-*` so a launcher start can turn off what the repo file turned on; `autoPushBranch`/`autoOpenPr` default ON so `false` must be said out loud (#1102); `autoMerge` defaults OFF but the repo file may turn it on, so its `false` travels too (#1216). Only vetted values are forwarded: ticket paths re-checked (`isTicketPath`), `via` must pass `isSafeVia`, queue entries passed verbatim via argv (never a shell).
