...
- **Auto PM's wiring** - which routines may run, how many agents at once, which tickets may be claimed, who writes the claims, and when a drained queue entry is checked off.
...
- **Stale pins.** A routine branch left pinned by a closed pull request blocks every later firing of that routine, so it is released.
