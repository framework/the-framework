...
- The CI watch (#1418, ci-watch.ts): merges `watched` PRs on green (ungated — it finishes a merge the run was armed and authorized for) and starts a CI-fix session on red, gated per attempt by the `autoPm` preference (the consent-to-spend-unasked switch) + quota headroom, started through `startUnattended` with push/PR/merge disarmed. Stopped in `quiesce` with the other run-starters.
...
- Auto PM ticks once at startup (#1161) so a daemon started with the setting on does not idle a full interval with quota going spare; `wakeAutoPm({onDemand: true})` (#1210) sweeps even while the preference is off, because the click is the ask. `wakeAutoPm` resolves when the tick does (#1433), so the dashboard's trigger can await the sweep's answer; fire-and-forget callers just drop the promise.
