...
- Starting an agent, previews, and opening a checkout in an editor exist only where the daemon wired them; other hosts refuse.
- Publishing a finished agent — push its branch, open a PR, merge — first commits what it left uncommitted, under a lock so a click racing teardown cannot lose work; Merge steers a live agent to merge at its natural end, and merges a finished one's PR directly.
