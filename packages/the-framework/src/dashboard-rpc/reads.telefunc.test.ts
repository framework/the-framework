...
import { onProjectFiles, onProjectFileStatus, onRunAnalysis, onRunWorktree } from './reads.telefunc.js'
...
test('onRunAnalysis degrades to null for an unknown project and an unsafe run id (#1180)', async () => {
  assert.equal(await onRunAnalysis('project-that-does-not-exist', '2026-07-19T10-00-00-000Z'), null)
  assert.equal(await onRunAnalysis('project-that-does-not-exist', '../../etc'), null)
})
