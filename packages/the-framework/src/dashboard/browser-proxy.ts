...
/**
 * The dashboard's route to an agent's browser preview (#813).
 *
 * The bridge itself lives in the agent (#802) on a port the OS picks per agent. The dashboard is a
 ...
 * and the agent's port is never named by the client: it comes from the agent's own meta. A caller
 ...
 * verbatim and only ever used to look an agent up, never as a path.
 */
...
/** How the proxy finds the agent's bridge. Injectable so a test needs no registry and no run. */
export type BrowserPortLookup = (projectId: string, agentId: string) => Promise<number | undefined>

/**
 * The real lookup: the port the agent recorded on its own meta. An agent with no browser, a finished
...
const defaultBrowserPortLookup: BrowserPortLookup = async (projectId, agentId) => {
  ...
  // Only a live agent: the bridge is torn down with the agent, so a port off a finished one would
  // reach whatever the OS handed that number next.
  return agent?.status === 'running' ? agent.browserStreamPort : undefined
}

/**
 * Proxy one request to the agent's bridge. Returns false when the URL is not a browser route, so
...
export async function handleBrowserProxy(
  ...
  if (!port) {
    // The pane polls this while an agent is starting, and an agent may never have a browser at all,
  ...
  // The agent can die mid-stream, which lands here rather than as a response.
  upstream.on('error', () => {
    if (!res.headersSent) res.writeHead(502, { 'content-type': 'text/plain' })
    res.end()
  })
  // Stop pulling frames the moment the pane goes away, or the agent keeps serving a dead viewer.
