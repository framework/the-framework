...
- `ghMergePr(cwd, number)` (#1216): `gh pr merge --squash --auto` first (the PR lands when its checks pass); where GitHub refuses auto-merge (repo setting off, or the PR already in clean status — both carry the `enablePullRequestAutoMerge` marker) it merges directly. A draft refusal means `gh pr ready` then one retry — the already-open path can find a draft a previous run left, and an armed merge is the statement its review already happened. Never throws; resolves an `AutoMergeOutcome` (`auto-armed` / `merged` / `failed`+error) the handoff reports.
