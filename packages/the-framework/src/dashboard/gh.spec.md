...
- `ghMergePr(cwd, number, gh?, opts?)` (#1216): `gh pr merge --squash --auto` first (the PR lands when its checks pass); where GitHub refuses auto-merge (repo setting off, or the PR already in clean status — both carry the `enablePullRequestAutoMerge` marker) the `whenUnarmed` option decides (#1418): `merge-now` (default, the human paths) merges directly; `watch` (the auto-handoff path) merges directly only when `ghPrCiStatus` already says `passing` and otherwise answers `watched` — the daemon's CI watch (ci-watch.ts) merges it on green. The checks read decides, not the refusal text: `clean status` is also what GitHub says while *non-required* checks still run, which is exactly the #1406 window; `none` defers too, because a suite takes seconds to attach after a push. A draft refusal means `gh pr ready` then one retry — the already-open path can find a draft a previous run left, and an armed merge is the statement its review already happened. Never throws; resolves an `AutoMergeOutcome` (`auto-armed` / `merged` / `watched` / `failed`+error) the handoff reports.
- `ghPrCiStatus(cwd, number)` (#1418): a PR's combined check state off `gh pr view --json statusCheckRollup,headRefOid,headRefName` — GitHub Actions check runs and classic commit statuses both. `passing` / `failing` (+ the failed checks' names, for the CI-fix prompt) / `pending` / `none`; skipped and neutral conclusions pass (matching GitHub's merge box), cancelled/timed-out do not, a concluded failure is `failing` whatever else still runs. Unreadable gh answers `none` — never a green. Carries `headSha` and `branch` so the fix half can pin its attempt and land its fix.
- `ghRepoAutoMerge(cwd)` (#1417): whether the repo allows GitHub auto-merge (`gh api repos/{owner}/{repo}` → `allow_auto_merge` — NOT `gh repo view --json autoMergeAllowed`, which no gh version supports and which shipped the warning dead), so the launcher can warn that an armed merge will degrade to an immediate direct merge (#1406). `known: false` when `gh` could not say (missing, unauthenticated, not a GitHub repo, or the field omitted for viewers without push access) — "could not say" is never presented as "off". `cachedRepoAutoMerge` is the #1028 cached form (5 min TTL — the launcher polls, the setting barely changes).
