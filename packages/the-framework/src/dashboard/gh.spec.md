...
- `ghRepoAutoMerge(cwd)` (#1417): whether the repo allows GitHub auto-merge (`gh repo view --json autoMergeAllowed`), so the launcher can warn that an armed merge will degrade to an immediate direct merge (#1406). `known: false` when `gh` could not say (missing, unauthenticated, not a GitHub repo) — "could not say" is never presented as "off". `cachedRepoAutoMerge` is the #1028 cached form (5 min TTL — the launcher polls, the setting barely changes).
