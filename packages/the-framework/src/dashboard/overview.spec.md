...
- `buildHotTickets` (#1139): tickets pooled and bucketed into three lanes — `in-progress` (a live run records the ticket #1117, else planned/spiked), `ai-queue` (an open `TODO_AGENTS.md` entry links to it), `high-priority` — lane-ordered, capped at 60; tickets in no lane are dropped (the card is a shortlist, not the backlog).
- `ticketBucket` is the lane decision; everything takes injectable readers and is forgiving of read failures.

## Problems

- "Someone planned this at some point" vs "this is being coded as you look": only `RunMeta.ticket` from a live run is hard evidence (`runId` set, #1117), and it exists for drain runs only — the plan/spike proxy still carries hand-worked tickets.

## Decisions

- Priority parsing: a bare number uses the ticket format's own 10–0 scale (10 = critical), so ≥7 is high — NOT the P0/P1 convention, whose low-numbers-first reading applies only when written `p0`/`p1`; getting that backwards kept `Priority: 8` backlogs off the card entirely.
- Lane precedence in-progress > ai-queue > high-priority: work under way outranks queued, which outranks a flag; a closed ticket is in no lane regardless of marks.
