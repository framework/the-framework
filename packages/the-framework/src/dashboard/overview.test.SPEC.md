...
- **Working now** — only running agents are surfaced, ordered by last activity with the most recent first; finished agents are left out. A `web`-target agent whose cloud session is still working or waiting on a bridged question is listed, marked with that state and keyed to the project's path; one whose work was adopted, one too old for its session to be alive, and one that was stopped are not.
