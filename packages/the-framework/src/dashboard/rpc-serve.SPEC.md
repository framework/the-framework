...
- The feed's response simply ending is how a viewer tells "the server is done" from "the connection dropped" — the first is a finished session, the second is worth retrying and worth saying out loud.
- A malformed request is answered, and so is one whose RPC throws: a failing call is a failing call, never a dead daemon.
