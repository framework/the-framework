...
- The merge half (#1216): `HandoffIntent.merge` (absent = off, no checkbox mutates it) has `runAutoHandoff` call `ghMergePr` on the PR it just opened (number off gh's URL, lookup as fallback) — and on the `already-open` skip's OPEN PR, which is a rerun/restart finding its predecessor's PR. Both calls pass `whenUnarmed: 'watch'` (#1418): where GitHub cannot arm the merge and the checks have not passed, the outcome is `watched` and the daemon's CI watch (ci-watch.ts) merges on green instead of the direct fallback landing it before CI (#1406). The outcome rides the handoff outcome as `merge`; a merge failure never fails the handoff (the PR is there either way).
- `mergeSessionPr` (#1391): the Merge action for a finished session — resolve the run's PR, refuse when there is none or it is no longer open, `ghMergePr` it (marking a draft ready on the way), and forget both PR caches under every candidate branch name. The human's answer to the withheld-merge ending (#1363).
