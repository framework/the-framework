...
- `readTickets(cwd)`: every non-sibling `.md` in `tickets/` as a `WorkspaceTicket` — title (`# ` heading, else the filename made readable), summary (the `## TLDR` line, else first prose line), preamble keys (`status:`/`priority:`/`topics:`/`GitHub:` above the title), date, `spiked`/`planned` (sibling `.spike.md`/`.plan.md` presence), and `effort` — newest first; `[]` when the dir is missing (the state the view offers to import into).
