All the data The Framework writes — the tickets, the task queue, the session archives — lives on one dedicated branch, `the-framework_data`, so the code history stays 100% code and the data can be pushed and pulled eagerly without ever touching anyone's working tree.

## TLDR

- The branch is checked out at `.the-framework/branches/the-framework_data`, and a `tickets` symlink at the repo root points into it, so the roadmap stays one `ls` away.
