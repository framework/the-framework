The driver seam: The Framework never calls a model of its own — it wraps a coding-agent CLI as a black box, and this subsystem is that wrapper. A driver is started once per agent, bound to the agent's workspace; each prompt lets the wrapped CLI's own loop run one full turn on the user's own subscription, and The Framework gates on outcomes — the turn's final message, the code produced, the exit — never on the agent's individual tool calls.
...
- **One seam, five implementations** - Claude Code locally, Codex locally, a Claude Code cloud session (the `web` run target's hand-off), a GitHub Actions workflow run per turn, and the scripted fake driver — all behind the same contract.
...
Claude Code local and Codex each spawn their CLI afresh per turn and share one process engine — spawn in an own process group, prompt over stdin, stream the output through the CLI's own dialect, gate on the exit — differing only in command line and output dialect. The `web` run target's implementation hands the whole task to a cloud session on claude.ai: hands-off, exactly one hand-off for the agent's life. The `actions` implementation runs each turn as a GitHub Actions workflow run, with continuity carried by the branch the previous run pushed. The fake driver replays scripted turns in memory for tests and offline demo runs. Each implementation carries a stable implementation id (`claude-code`, `codex`, `claude-web`, `github-actions`, `fake`); one driver has an implementation per place it can run, and the id maps back to the user's driver choice.
...
Claude Code runs on the user's Claude subscription and Codex on their ChatGPT subscription; the CLI authenticates itself, so The Framework never reads or holds a token. The cloud session draws on the same Claude account, and a workflow run authenticates with an OAuth token the repo holds, minted by the user's own `claude setup-token`. What a turn spent is reported as usage (tokens always, a price only when the CLI prices its turns); where the account's quota stands is a separate account-wide read that only the Claude Code driver can answer — a driver that cannot answer omits the ability rather than fake a number.
