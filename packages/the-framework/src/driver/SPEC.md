...
- **The resume handle.** The session id is captured from the first thing the CLI says, not from the end of the turn. A resume that fails because the CLI forgot the conversation silently reruns fresh, with a notice, rather than losing the message the user already sent.
