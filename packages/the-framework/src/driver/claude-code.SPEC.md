...
- File edits are auto-accepted by default so runs never stall on a permission prompt; skipping the agent's permission system entirely is a separate, explicit opt-in meant for sandboxes.
