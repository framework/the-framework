...
- Events are the run's durable record: anything a dashboard tab opened later must know (the ticket being implemented, the branch, the pull request opened for the work, what the end-of-session handoff is armed to do) travels as an event, because only events reach the run's stored history.
