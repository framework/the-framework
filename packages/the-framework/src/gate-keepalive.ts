/**
 * Holds the Node event loop open while the run is parked on the user (#1359).
 *
 * A daemon-spawned run has nothing ref'd at a parked gate: the daemon spawns it detached with
 ...
 * deliberately unref'd so steering alone never keeps a finished run alive (#344). The moment
 ...
 * only while at least one held promise is pending. Waiting for the answer IS the run's work at
...
/** A live keepalive. One per run process; every parked wait shares it. */
...
/**
 * Create the run's keepalive. Counter-based: the first pending hold starts one ref'd idle
