...
- A live session keeps its checkout: its agent is working in there, and Stop is how a run ends. So does one the daemon has not finished retiring — a run's records say `done` a beat before its teardown reclaims the checkout, and "not live" is not "nobody is holding this".
