...
- The triage pair splits on cost (quick wins by the plan's own effort/uncertainty numbers vs. significant-only), both append to the agent queue, both end with the shared queue-only rule verbatim (only `TODO_AGENTS.md` may change), each pins its own distinct session name and aborts when its branch already exists, and neither ever gates — while the gated sibling still does.
