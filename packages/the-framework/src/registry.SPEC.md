...
- One tier of preferences lives here, and it is the user's. Repo-shaped settings belong in the repo's committed file, so a per-project block in a home file was a second answer to the same question that only one machine could see. The block that tier left behind is read once, by the notice that names it, and never by anything that acts on it.
