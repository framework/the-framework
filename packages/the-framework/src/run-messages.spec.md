...
- `RunMessages.next(signal)`: the run loop drains queued messages between turns and, when the agent goes idle, waits here for the next one (the "stay-open" chat lifecycle); resolves `undefined` when the run should stop waiting (signal aborted, or source closed).
