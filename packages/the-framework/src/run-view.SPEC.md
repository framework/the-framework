Distills a run's event stream into the dashboard's summary cards: the run's lifecycle progress, what it will do with its work when it ends, and the agent session behind it.
...
- Latest wins throughout: the agent may rename its session or re-arm its handoff mid-run.
