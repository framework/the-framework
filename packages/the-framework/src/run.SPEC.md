...
- Review is opt-in: a chosen domain preset reviews the build (its bug-fix or major-change loop, as the run chose). Without one, the build turn is the whole run: the agent is a black box unless the user opted into reviewing it.
...
- Detect and narrate → frame the session → scope → build (pausing on the agent's questions) → review-and-improve until clean, when configured → backlog loop → live chat → end.
