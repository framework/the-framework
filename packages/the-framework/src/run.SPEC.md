...
- Review is opt-in: a chosen domain preset reviews the build (its bug-fix or major-change loop, as the run chose), optionally joined by a boot-and-serve check that gates on the app actually running — sandboxed in a throwaway container if asked. With neither, the build turn is the whole run: the agent is a black box unless the user opted into reviewing it.
