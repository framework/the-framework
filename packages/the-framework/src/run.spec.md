The build-run orchestrator: `runFramework()` drives ai-autopilot's `Bootstrap` (scope → build → review loop when one is configured → deploy) entirely through a `Driver` session, then the backlog loop and live chat, streaming every phase as `FrameworkEvent`s.
...
- Build continuation (#1467): `resumeSessionId` resumes a stopped leg's conversation (passed to `driver.start`) and sends `intent` — the continuation message — **verbatim** as the build turn (`driverBuild`'s prompt override), because the resumed transcript already carries the scope→build framing (#782's objection). Everything around the turn still runs: bootstrap narration with synthesize, checklist where configured, backlog loop, chat — the flow resumes, not just the conversation.
- Detection only narrates ("Detected Vike") — nothing about it reaches the agent's prompt (#547).

## Problems

- Hand-off drivers (#1225): the prompt leaves the machine and the reply never comes back, so every phase after build would read the driver's own "handed off to <url>" summary as agent output — producing a bogus blocker and an unanswerable backlog gate. Fix: with `driver.handsOff`, checklist/improve steps are omitted (Bootstrap then skips its whole loop), the todo loop and chat phase are skipped, and a closing log explains the run continues in its own session.
- Stops with nothing after the build (#1372): the budget/quota/decline controls abort between phases, and the review loop used to be the phase that observed it. Now `agentAwaitGate` throws at the end of the build step when the run signal aborted, and `runFramework` re-checks after `bootstrap.run()` — otherwise an aborted run settles as `done`/`ok: true`.
...
- run: detect preset → compose system → `session` + `system-prompt` + detection/modes logs → controls → `driver.start` → buildReview → provision serve runner → `Bootstrap.run` (scope → build[awaitGate] → checklist+improve loop when a preset/serve wired one → deploy) → todo loop → keepAlive preview → chat phase → `end {ok:true}`.
