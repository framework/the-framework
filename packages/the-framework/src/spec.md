...
- **Queue + autonomy** — `queue-promote.ts` (queued prompts → runs), `auto-pm.ts` (the autonomous PM loop: drain the queue / maintenance on quota headroom), `planned-quick-wins.ts` (a plan's own verdict → a queue entry, #1334).
