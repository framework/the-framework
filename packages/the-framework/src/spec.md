...
- **Queue + autonomy** — `queue-promote.ts` (queued prompts → runs), `auto-pm.ts` (the autonomous PM loop: drain the queue / maintenance on quota headroom). A plan's own verdict reaches the queue via the planning agent itself (#1334/#1420: the [Spike & plan] preset's queued entries tell it to queue an unambiguous quick win in the same commit as the plan).
