...
- Prompt composers: `buildPrompt` (greenfield), `extendPrompt` (#185, names the existing codebase and the intent; the how-to-behave rules were dropped in #1224; asks for the `set-scope` verdict, #1356 — a trivial run has no sizing moment of its own, and greenfield/scaffold builds deliberately don't ask because from-scratch is never small), `scaffoldPrompt` (#182, hard "create from scratch" directive), `PRODUCTION_GRADE_PROMPT` + `improvePrompt`.
