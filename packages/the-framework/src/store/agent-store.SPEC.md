...
- Finished agents are archived into the repo under per-user directories and committed, so a project's history survives a `git clean`; every user's archive is listed (the history is a team record), duplicates show once, and an agent's live copy beats its archived one — a continued agent is both at once.
