...
The branch is recorded as the agent observes it, and corrected at teardown while the worktree still exists, rather than being derived. It cannot be derived: a finished agent loses its checkout, and the agent is free to create its own branch, so neither the session-name branch nor the agent-id branch is guaranteed to hold the commits.
