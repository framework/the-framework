...
test('listAgents reads every user archive and the transient one, under their one name (#1179)', async () => {
  ...
  const user = join(CWD, '.the-framework', 'branches', 'the-framework_data', 'agents', 'dev@example.com')
  const fs = memFs({
...
const archiveAt = (id: string, ext: string) =>
  join(CWD, '.the-framework', 'branches', 'the-framework_data', 'agents', USER, `${id}.${ext}`)
...
test('the history lists every user, and the runs archived before this shipped (#1179)', async () => {
  ...
  const fs = memFs({
    ...
    [join(CWD, '.the-framework', 'branches', 'the-framework_data', 'agents', 'someone@else.com', 'r3.json')]: done('r3'),
  })
