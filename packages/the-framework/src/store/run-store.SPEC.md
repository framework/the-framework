...
- The snapshot is rewritten in place by the session's own process while everyone else polls it, so a read caught inside one of those writes is retried rather than reported as an absent session — a live session must never blink out of a listing, and readers act on that absence.
