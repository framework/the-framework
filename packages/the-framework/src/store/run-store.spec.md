...
- A death mid-gate leaves an eternal question (#1359): a run that died without writing `end` never wrote `choice-resolved` either, so a bare status flip left `pendingChoice` on the meta and the dashboard rendering the dead run's question as answerable forever. Every flip of a dead `running` run now goes through `recordOrphanEnd`: append the surrogate `orphanEndEvent()` (`end`, ok false, stopped) to the log the reader replays, and fold it via `applyEventToMeta` so pendingChoice/settledAt/browserStreamPort clear exactly as a run-written end clears them. Applied at all four flips: `stopAndArchiveLive` (read-time self-heal + boot reconcile's live run), `archiveWorktreeRun`'s running branch (into the worktree's own log before the copy), and both reconcile branches (the archived pair gets the event in its `<id>.jsonl` sibling).
