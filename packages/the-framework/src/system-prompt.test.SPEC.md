...
- The browser protocol appears only when the agent has a browser; the hands-off protocol (land everything) appears only for a hands-off agent; the decide-alone protocol appears only for an agent nothing can answer, directly after the await protocol and ahead of the hands-off protocol — so a hands-off agent whose questions do reach the user keeps the same await protocol a local agent gets. A local agent gets none of them. All survive vanilla and none survives transparent.
