Renders the run's event stream as one human-readable terminal line per event — the CLI's counterpart to the dashboard's views over the same stream.
...
- An agent that reports no price shows its token counts rather than a $0 that would read as free.
