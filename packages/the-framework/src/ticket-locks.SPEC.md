...
- No timed expiry — an agent may legitimately hold a ticket for days. The lock lifts when the agent's own PR deletes it, when a human releases it, or when the daemon frees a claim it made for an agent that ended with nothing to hand off — the one claim it can know is dead rather than guess by a clock. A release whose commit fails puts the file back, and the daemon only ever frees a lock still naming the exact agent it made it for.
